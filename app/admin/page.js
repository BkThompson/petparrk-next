...
export default function AdminPage() {
  ...
  const [addPriceForm, setAddPriceForm] = useState({
    ...
    species: "dog", species_other: "", call_for_quote: false, notes: "",
  });
  ...
  async function fetchCallQueue() {
    ...
    const activeMissingPrices = (activeVets || []).filter(v => !vetsWithPrices.has(v.id)).map(v => ({ ...v, _source: "active" }));
    const pending = (pendingVetsList || []).map(v => ({ ...v, _source: "pending" }));
  ...
  async function saveCallPrices(vet) {
    ...
    if (vet._source === "pending") {
      ...
      const { data: newVet, error: vetError } = await supabase.from("vets").insert({
        name: vet.name, slug, address: vet.address, city: vet.city,
  ...
  function addCallPriceRow() {
    setCallPrices(prev => [...prev, {
      ...
      species: "dog", call_for_quote: false, notes: "",
    }]);
  }
  ...
  async function fetchPricesForVet(vetId) {
    ...
    setVetPrices(data || []); setPricesLoading(false);
  }
  ...
  async function approvePendingVet(vet) {
    ...
    const { error } = await supabase.from("vets").insert({
      name: form.name, slug, address: form.address, city: form.city,
  ...
  async function saveVet() {
    ...
    const { error } = await supabase.from("vets").update({
      ...
      city: vetForm.city, address: vetForm.address, zip_code: vetForm.zip_code,
  ...
  // ── Price actions ─────────────────────────────────────────────────
  function startEditPrice(price) { setEditingPrice(price.id); setPriceForm({ ...price }); }
  ...
  async function addPrice() {
    ...
    if (!error) {
      ...
      setAddPriceForm({ service_id: "", price_low: "", price_high: "", price_type: "exact", includes_bloodwork: false, includes_xrays: false, includes_anesthesia: false, species: "dog", species_other: "", call_for_quote: false, notes: "" });
      fetchStats();
    } else { alert("Error: " + error.message); }
  }
    ...
    return (
      <div className="row-edit-bg" style={{ marginTop: "10px" }}>
        <div className="form-grid-3" style={{ marginBottom: "10px" }}>
          {[["name","Name"],["address","Address"],["city","City"],["zip_code","ZIP"],["phone","Phone"],["website","Website"],["neighborhood","Neighborhood"],["vet_type","Vet Type"],["hours","Hours"]].map(([f, label]) => (
            <div key={f}>
      ...
      <div style={{ maxWidth: "900px", margin: "0 auto", padding: "20px", fontFamily: "system-ui, sans-serif", minHeight: "100vh", background: "#f7f7f5" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
          <div>
            ...
            <p style={{ margin: 0, fontSize: "12px", color: "#888" }}>Signed in as {session?.user?.email}</p>
          </div>
          ...
          <div style={{ padding: "20px" }}>
                  ...
                  <div key={s.id} className="sub-card">
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "8px" }}>
                      <div>
                        ...
                        {s.submitter_note && <p style={{ margin: "4px 0 0 0", fontSize: "12px", color: "#666", fontStyle: "italic" }}>"{s.submitter_note}"</p>}
                        <p style={{ margin: "4px 0 0 0", fontSize: "11px", color: "#aaa" }}>Submitted {formatDate(s.created_at)}</p>
                      </div>
              ...
              <div>
                <div className="section-header">
                  <div>
                    ...
                    <p style={{ margin: 0, fontSize: "12px", color: "#888" }}>Found by the AI agent — review before going live</p>
                  </div>
                </div>
                ...
                {!pendingVetsLoading && pendingVets.length === 0 && (
                  <div style={{ textAlign: "center", padding: "48px 20px" }}>
                    ...
                    <p style={{ color: "#bbb", fontSize: "14px", margin: 0 }}>No pending vets. The AI agent hasn't found any new ones yet.</p>
                  </div>
                )}
                {pendingVets.map(vet => (
                  <div key={vet.id} className="pending-vet-card">
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "8px", marginBottom: editingPendingVet === vet.id ? "4px" : "0" }}>
                      <div>
                        ...
                        <p style={{ margin: 0, fontSize: "12px", color: "#888" }}>{[vet.address, vet.city, vet.zip_code].filter(Boolean).join(", ")}</p>
                        {vet.phone && <p style={{ margin: "2px 0 0 0", fontSize: "12px", color: "#888" }}>{vet.phone}</p>}
                        <p style={{ margin: "4px 0 0 0", fontSize: "11px", color: "#bbb" }}>Found {formatDate(vet.created_at)}</p>
                      </div>
            ...
            {tab === "Vets" && (
              <div>
                ...
                <input className="adm-input" value={vetSearch} onChange={e => setVetSearch(e.target.value)} placeholder="Search vets by name or neighborhood..." style={{ marginBottom: "14px", maxWidth: "360px" }} />
                      ...
                      <div className="vet-row" style={{ padding: "10px 14px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "10px" }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: "12px", color: "#888", marginTop: "2px" }}>{vet.neighborhood}</div>
                            <div style={{ fontSize: "12px", color: "#aaa", marginTop: "1px" }}>{vet.phone}</div>
                          </div>
                          <div className="vet-row-buttons" style={{ display: "flex", gap: "6px", alignItems: "center", flexShrink: 0 }}>
                            <span className={`badge badge-${vet.status}`}>{vet.status}</span>
                      ...
                      <div className="row-edit-bg" style={{ marginBottom: "14px" }}>
                        ...
                        <div className="form-grid-3" style={{ marginBottom: "10px" }}>
                          <div>
                            ...
                            <select className="adm-input" value={addPriceForm.species || "dog"} onChange={e => setAddPriceForm({ ...addPriceForm, species: e.target.value, species_other: "" })}>
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: "6px", paddingTop: "18px" }}>
                            {[["includes_bloodwork","Includes bloodwork"],["includes_xrays","Includes x-rays"],["includes_anesthesia","Includes anesthesia"],["call_for_quote","Call for quote"]].map(([f, l]) => (
                              <label key={f} style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", cursor: "pointer" }}>
                                <input type="checkbox" checked={!!addPriceForm[f]} onChange={e => setAddPriceForm({ ...addPriceForm, [f]: e.target.checked })} />{l}
                              </label>
                            ))}
                          </div>
                          <div><label className="field-label">Notes</label><input className="adm-input" value={addPriceForm.notes} onChange={e => setAddPriceForm({ ...addPriceForm, notes: e.target.value })} placeholder="Optional" /></div>
                        </div>
                        <button className="adm-btn adm-btn-green" onClick={addPrice}>Add Price</button>
                      </div>
                    )}
                      ...
                      {vetPrices.map(p => (
                        <div key={p.id}>
                          <div className="price-row" style={{ padding: "10px 14px" }}>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontWeight: "600", fontSize: "13px", color: "#111", marginBottom: "3px" }}>{p.services?.name || "—"}</div>
                              <div><span style={{ fontSize: "13px", color: "#2d6a4f", fontWeight: "600" }}>{formatPrice(p.price_low, p.price_high, p.price_type)}</span><span style={{ marginLeft: "6px", fontSize: "11px", color: "#888" }}>({p.price_type})</span></div>
                              <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", marginTop: "4px" }}>
                                {p.species && <span style={{ fontSize: "11px", background: "#f0f0f0", color: "#555", padding: "1px 6px", borderRadius: "4px", textTransform: "capitalize" }}>{p.species}</span>}
                                {p.includes_bloodwork && <span style={{ fontSize: "11px", background: "#e8f5e9", color: "#2d6a4f", padding: "1px 6px", borderRadius: "4px" }}>+ bloodwork</span>}
                                {p.includes_xrays && <span style={{ fontSize: "11px", background: "#e8f5e9", color: "#2d6a4f", padding: "1px 6px", borderRadius: "4px" }}>+ x-rays</span>}
                                {p.includes_anesthesia && <span style={{ fontSize: "11px", background: "#e8f5e9", color: "#2d6a4f", padding: "1px 6px", borderRadius: "4px" }}>+ anesthesia</span>}
                                {p.call_for_quote && <span style={{ fontSize: "11px", background: "#fff8e1", color: "#e65100", padding: "1px 6px", borderRadius: "4px" }}>call for quote</span>}
                                {p.notes && <span style={{ fontSize: "11px", color: "#888", fontStyle: "italic" }}>{p.notes}</span>}
                              </div>
                            </div>
                            <div style={{ display: "flex", gap: "6px", flexShrink: 0, alignSelf: "flex-start" }}>
                              <button className="adm-btn adm-btn-outline" onClick={() => editingPrice === p.id ? setEditingPrice(null) : startEditPrice(p)}>{editingPrice === p.id ? "Cancel" : "Edit"}</button>
                              <button className="adm-btn adm-btn-red" onClick={() => deletePrice(p.id)}>Delete</button>
                            </div>
                          </div>
                          {editingPrice === p.id && (
                            <div style={{ padding: "0 14px 14px 14px" }}>
                              <div className="row-edit-bg">
                                ...
                                <div className="form-grid-3" style={{ marginBottom: "10px" }}>
                                  ...
                                  <div style={{ display: "flex", flexDirection: "column", gap: "6px", paddingTop: "18px" }}>
                                    {[["includes_bloodwork","Includes bloodwork"],["includes_xrays","Includes x-rays"],["includes_anesthesia","Includes anesthesia"],["call_for_quote","Call for quote"]].map(([f, l]) => (
                                      <label key={f} style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", cursor: "pointer" }}>
                                        <input type="checkbox" checked={!!priceForm[f]} onChange={e => setPriceForm({ ...priceForm, [f]: e.target.checked })} />{l}
                                      </label>
                                    ))}
                                  </div>
                                  <div><label className="field-label">Notes</label><input className="adm-input" value={priceForm.notes || ""} onChange={e => setPriceForm({ ...priceForm, notes: e.target.value })} /></div>
                                </div>
                                <div style={{ display: "flex", gap: "8px" }}>
                                  <button className="adm-btn adm-btn-green" onClick={savePrice} disabled={priceSaving}>{priceSaving ? "Saving..." : "Save Changes"}</button>
            ...
            {tab === "Call Sheet" && (
              <div>
                ...
                {!callQueueLoading && callIndex >= callQueue.length && callQueue.length > 0 && (
                  <div style={{ textAlign: "center", padding: "48px 20px" }}>
                    <p style={{ fontSize: "32px", margin: "0 0 8px 0" }}>✅</p>
                ...
                {!callQueueLoading && callIndex < callQueue.length && (() => {
                  ...
                  return (
                    <div>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                        <div>
                          <h2 style={{ margin: "0 0 2px 0", fontSize: "1rem", color: "#111" }}>Call Sheet</h2>
                          <p style={{ margin: 0, fontSize: "12px", color: "#888" }}>{callIndex + 1} of {callQueue.length} vets</p>
                        </div>
                        <div style={{ display: "flex", gap: "8px" }}>
                      ...
                      {/* Progress bar */}
                      <div style={{ height: "4px", background: "#f0f0f0", borderRadius: "4px", marginBottom: "20px" }}>
                        <div style={{ height: "4px", background: "#2d6a4f", borderRadius: "4px", width: `${((callIndex + 1) / callQueue.length) * 100}%`, transition: "width 0.3s" }} />
                      </div>
                      ...
                      <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: "12px", padding: "20px", marginBottom: "16px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "8px", marginBottom: "12px" }}>
                          <div>
                            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                              <h3 style={{ margin: 0, fontSize: "1.1rem", color: "#111", fontWeight: "700" }}>{vet.name}</h3>
                            ...
                            <p style={{ margin: "0 0 2px 0", fontSize: "13px", color: "#666" }}>{[vet.address, vet.city, vet.zip_code].filter(Boolean).join(", ")}</p>
                            {vet.website && <a href={vet.website} target="_blank" rel="noreferrer" style={{ fontSize: "12px", color: "#2d6a4f" }}>{vet.website}</a>}
                          </div>
                          {vet.phone && (
                            <a href={`tel:${vet.phone}`} style={{ display: "flex", alignItems: "center", gap: "6px", background: "#2d6a4f", color: "#fff", padding: "8px 16px", borderRadius: "8px", textDecoration: "none", fontWeight: "700", fontSize: "14px" }}>
                              📞 {vet.phone}
                            </a>
                          )}
                        </div>
                        ...
                        {/* Status buttons */}
                        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "16px", paddingBottom: "16px", borderBottom: "1px solid #f0f0f0" }}>
                          <span style={{ fontSize: "12px", color: "#888", alignSelf: "center" }}>No prices?</span>
                          ...
                          <button className="adm-btn adm-btn-red" onClick={() => markCallStatus(vet, "skip")}>✕ Skip</button>
                        </div>
                        ...
                        <div>
                          ...
                          {callPrices.map((p, i) => (
                            <div key={i} className="row-edit-bg" style={{ marginBottom: "8px" }}>
                              <div className="form-grid-4" style={{ marginBottom: "8px" }}>
                                <div>
                                  <label className="field-label">Service</label>
                                  <select className="adm-input" value={p.service_id} onChange={e => updateCallPrice(i, "service_id", e.target.value)}>
                                ...
                                <div>
                                  ...
                                  <select className="adm-input" value={p.price_type} onChange={e => updateCallPrice(i, "price_type", e.target.value)}>
                                    {PRICE_TYPES.map(t => <option key={t}>{t}</option>)}
                                  </select>
                                </div>
                                <div>
                                  <label className="field-label">Price Low</label>
                                  <input className="adm-input" type="number" value={p.price_low} onChange={e => updateCallPrice(i, "price_low", e.target.value)} placeholder="e.g. 65" />
                                </div>
                                <div>
                                  <label className="field-label">Price High</label>
                                  <input className="adm-input" type="number" value={p.price_high} onChange={e => updateCallPrice(i, "price_high", e.target.value)} placeholder="range only" />
                                </div>
                              </div>
                              <div style={{ display: "flex", gap: "16px", alignItems: "center", flexWrap: "wrap" }}>
                                <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", cursor: "pointer" }}>
                                  <input type="checkbox" checked={p.includes_bloodwork} onChange={e => updateCallPrice(i, "includes_bloodwork", e.target.checked)} />Bloodwork
                                </label>
                                <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", cursor: "pointer" }}>
                                  <input type="checkbox" checked={p.includes_xrays} onChange={e => updateCallPrice(i, "includes_xrays", e.target.checked)} />X-rays
                                </label>
                                <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", cursor: "pointer" }}>
                                  <input type="checkbox" checked={p.includes_anesthesia} onChange={e => updateCallPrice(i, "includes_anesthesia", e.target.checked)} />Anesthesia
                                </label>
                                <div style={{ flex: 1, minWidth: "120px" }}>
                                  <input className="adm-input" value={p.notes} onChange={e => updateCallPrice(i, "notes", e.target.value)} placeholder="Notes..." />
                                </div>
                                <button className="adm-btn adm-btn-red" onClick={() => removeCallPrice(i)}>✕</button>
                              </div>
                            </div>
                          ))}

                          {callPrices.length > 0 && (
                            <button className="adm-btn adm-btn-green" style={{ marginTop: "8px", padding: "8px 20px", fontSize: "13px" }} onClick={() => saveCallPrices(vet)} disabled={callSaving}>
                              {callSaving ? "Saving..." : "✅ Save & Next Vet"}
                            </button>
                          )}
                        </div>
                      </div>
            ...
            {tab === "Users" && (
              <div>
                ...
                <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: "8px", overflow: "hidden" }}>
                  <div className="table-header" style={{ gridTemplateColumns: "2fr 1fr 1fr 1fr", display: "grid" }}>
                  ...
                  {filteredUsers.map(u => (
                    <div key={u.id} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", padding: "10px 14px", borderBottom: "1px solid #f5f5f3", alignItems: "center", gap: "8px" }}>
                      <div>
                        <span style={{ fontSize: "13px", fontWeight: "600", color: "#111" }}>{u.full_name || <span style={{ color: "#bbb", fontStyle: "italic" }}>No name</span>}</span>
                        {u.bio && <p style={{ margin: "2px 0 0 0", fontSize: "11px", color: "#aaa", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{u.bio}</p>}
                      </div>
                      <span style={{ fontSize: "13px", color: "#666" }}>{u.zip_code || "—"}</span>
                      <span style={{ fontSize: "12px" }}>{u.is_public ? <span style={{ color: "#2d6a4f" }}>✅ Public</span> : <span style={{ color: "#bbb" }}>Private</span>}</span>
                      <span style={{ fontSize: "12px", color: "#bbb" }}>{formatDate(u.created_at)}</span>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  {Object.entries(TRIAGE_CONFIG).map(([key, cfg]) => {
                    ...
                    return (
                      <button key={key} onClick={() => setSymptomFilter(active ? "all" : key)}
                        style={{ background: active ? cfg.color : cfg.bg, border: `1px solid ${cfg.color}44`, borderRadius: "10px", padding: "10px 16px", cursor: "pointer", textAlign: "left", minWidth: "100px", transition: "all 0.15s" }}>
                        <p style={{ margin: "0 0 2px 0", fontSize: "18px" }}>{cfg.emoji}</p>
                        <p style={{ margin: "0 0 2px 0", fontSize: "20px", fontWeight: "700", color: active ? "#fff" : cfg.color }}>{count}</p>
                  ...
                  {filteredLogs.map(s => {
                    ...
                    return (
                      <div key={s.id} className="log-row">
                        ...
                        <span style={{ fontSize: "13px", color: "#666", textTransform: "capitalize" }}>{s.pets?.species || "—"}</span>
