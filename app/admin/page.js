...
export default function AdminPage() {
  ...
  async function fetchUnverifiedPrices() {
    ...
    const { data } = await supabase.from("vet_prices").select("*, services(name), vets(name), pending_vets(name)").eq("is_verified", false).eq("source", "ai_scraper").order("created_at", { ascending: false });
  ...
  async function fetchCallQueue() {
    ...
    const pending = (pendingVetsList || []).map((v) => ({ ...v, _source: "pending", _hasPrices: false, _declined: v.notes === "declined_to_share" }));
    const activeAll = (activeVets || []).map((v) => ({ ...v, _source: "active", _hasPrices: vetsWithPrices.has(v.id), _declined: v.internal_notes === "declined_to_share" }));
    const activeMissing = activeAll.filter((v) => !v._hasPrices && !v._declined);
  ...
  async function saveCallPrices(vet) {
    const targetVet = lockedVetId && lockedVetId !== vet.id ? { ...vet, id: lockedVetId, name: lockedVetName } : vet;
    ...
    if (vetToSave._source === "pending" && !callReviewVetId) {
      const { data: existingVet } = await supabase.from("vets").select("id").ilike("name", vetToSave.name.trim()).maybeSingle();
      if (existingVet) {
        savedVetId = existingVet.id;
        await supabase.from("pending_vets").update({ status: "approved" }).eq("id", vetToSave.id);
      } else {
        const slug = (vetToSave.slug || vetToSave.name).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
        const { data: newVet, error: vetError } = await supabase.from("vets").insert({
          ...
          neighborhood: vetToSave.neighborhood || null,
          vet_type: vetToSave.vet_type ? Array.isArray(vetToSave.vet_type) ? vetToSave.vet_type : [vetToSave.vet_type] : ["General Practice"],
          accepting_new_patients: null,
          carecredit: false,
          status: "inactive",
        }).select().single();
      ...
      const updatedVet = { ...vetToSave, id: savedVetId, _source: "active", _hasPrices: true };
      setCallQueue((prev) => prev.map((v) => v.id === vetToSave.id && v._source === "pending" ? updatedVet : v));
      setFullCallQueue((prev) => prev.map((v) => v.id === vetToSave.id && v._source === "pending" ? updatedVet : v));
    }
  ...
  async function updateReviewPrice(index, form) {
    ...
    const { error } = await supabase.from("vet_prices").update({
      ...
      species: form.species === "other" ? form.speciesOther || "other" : form.species || null,
  ...
  async function markCallStatus(vet, status) {
    if (vet._source === "pending") {
      if (status === "skip") {
        ...
        await supabase.from("pending_vets").update({ notes: "declined_to_share" }).eq("id", vet.id);
      } else {
        await supabase.from("pending_vets").update({ notes: status }).eq("id", vet.id);
      }
    } else {
      if (status === "declined") {
        await supabase.from("vets").update({ internal_notes: "declined_to_share" }).eq("id", vet.id);
      } else {
        await supabase.from("vets").update({ internal_notes: status }).eq("id", vet.id);
      }
    }
    if (status === "declined") {
      setCallQueue((prev) => prev.map((v, idx) => idx === callIndex ? { ...v, _declined: true } : v));
      setFullCallQueue((prev) => prev.map((v, idx) => idx === callIndex ? { ...v, _declined: true } : v));
    }
    setCallIndex((i) => i + 1);
  }

  function addCallPriceRow(vet) {
    if (vet && !lockedVetId) { setLockedVetId(vet.id); setLockedVetName(vet.name); }
    setCallPrices((prev) => [...prev, {
      ...
      species: "", call_for_quote: false, notes: "", vaccines_included: "",
    }]);
  }
  ...
  async function fetchReviewPrices(vetId) {
    ...
    const { data } = await supabase.from("vet_prices").select("*, services(name)").eq("vet_id", vetId).order("created_at");
    const clean = (data || []).map((p) => ({ ...p, includes_bloodwork: toBool(p.includes_bloodwork), includes_xrays: toBool(p.includes_xrays), includes_anesthesia: toBool(p.includes_anesthesia) }));
    setCallReviewPrices(clean);
  }

  async function fetchAllCallNotes() {
    setAllNotesLoading(true);
    const { data } = await supabase.from("call_notes").select("*").order("created_at", { ascending: false });
  ...
  async function fetchCallNotes(vetId) {
    if (!vetId) return;
    const { data } = await supabase.from("call_notes").select("*").eq("vet_id", vetId).order("created_at", { ascending: false });
    setCallNotes(data || []);
  }

  async function saveCallNote(vetId, vetName, text) {
    if (!text.trim()) return;
    const { data, error } = await supabase.from("call_notes").insert({ vet_id: vetId, vet_name: vetName, note: text.trim() }).select().single();
  ...
  async function updateCallNote(id, text) {
    if (!text.trim()) return;
    const { error } = await supabase.from("call_notes").update({ note: text.trim(), updated_at: new Date().toISOString() }).eq("id", id);
    if (error) { alert("Error updating note: " + error.message); return; }
    setCallNotes((prev) => prev.map((n) => n.id === id ? { ...n, note: text.trim() } : n));
    setAllCallNotes((prev) => prev.map((n) => n.id === id ? { ...n, note: text.trim() } : n));
    setEditingNoteId(null); setEditingNoteText("");
    setAllNotesEditingId(null); setAllNotesEditingText("");
  }
  ...
  async function deleteCallNote(id) {
    ...
    setDeletingNoteId(null); setAllNotesDeletingId(null);
  }
  ...
  async function inviteAdminUser() {
    ...
    const { error } = await supabase.from("admin_users").insert({
      ...
      status: "active",
      can_view_call_sheet: false, can_edit_prices: false,
      can_approve_vets: false, can_manage_users: false, can_manage_team: false,
      invited_by: currentUserEmail,
    });
    if (error) { setInviteError(error.message); setInviteSaving(false); return; }
    setInviteForm({ email: "", full_name: "" });
    setShowInviteForm(false); setInviteSaving(false);
    fetchAdminUsers();
  }

  async function updateAdminPermission(id, field, value) {
    await supabase.from("admin_users").update({ [field]: value }).eq("id", id);
    setAdminUsers((prev) => prev.map((u) => u.id === id ? { ...u, [field]: value } : u));
  }

  async function updateAdminName(id, name) {
    await supabase.from("admin_users").update({ full_name: name.trim() || null }).eq("id", id);
    setAdminUsers((prev) => prev.map((u) => u.id === id ? { ...u, full_name: name.trim() || null } : u));
    setTeamEditingId(null); setTeamEditName("");
  }

  async function toggleAdminStatus(user) {
    const newStatus = user.status === "active" ? "inactive" : "active";
    await supabase.from("admin_users").update({ status: newStatus }).eq("id", user.id);
  ...
  async function fetchPricesForVet(vetId) {
    ...
    setVetPrices([]);
    const { data, error } = await supabase.from("vet_prices").select(`*, services(name), _ts:created_at`).eq("vet_id", vetId).order("created_at").limit(200);
  ...
  async function approvePendingVet(vet) {
    ...
    const { error } = await supabase.from("vets").insert({
      ...
      hours: form.hours,
      // ── neighborhood stored as-is (no longer stripped when it matches city) ──
  ...
  async function saveVet() {
    ...
    if (!error) {
      setVets((prev) => prev.map((v) => v.id === editingVet ? { ...v, ...vetForm, website: cleanWebsiteUrl(vetForm.website) } : v));
      setEditingVet(null);
    } else {
  ...
  async function addVet() {
    ...
    const { data, error } = await supabase.from("vets").insert({
      ...
      vet_type: Array.isArray(addVetForm.vet_type) ? addVetForm.vet_type : [addVetForm.vet_type],
    }).select().single();
    if (!error) {
      setVets((prev) => [...prev, data].sort((a, b) => a.name.localeCompare(b.name)));
      setShowAddVet(false);
      setAddVetForm({
        name: "", slug: "", neighborhood: "", city: "Oakland", address: "",
        zip_code: "", phone: "", website: "", vet_type: ["General Practice"],
        ownership: "Independent", accepting_new_patients: null, carecredit: false,
        hours: "", status: "active", internal_notes: "",
      });
      fetchStats();
    } else {
  ...
  function startEditPrice(price) {
    ...
    setPriceForm({ ...price, price_low: price.price_low != null ? String(price.price_low) : "", price_high: price.price_high != null ? String(price.price_high) : "" });
  }

  async function savePrice(formData) {
    const f = formData || priceForm;
    if (!f.service_id || !f.species || (!f.price_low && !f.call_for_quote)) { setEditPriceError(true); return; }
    setEditPriceError(false); setPriceSaving(true);
    const payload = {
  ...
  async function deletePrice(id) {
    ...
    setDeletePriceConfirm(null); fetchStats();
    if (callReviewVetId === selectedVetId) fetchReviewPrices(callReviewVetId);
  }

  async function addPrice() {
    if (!addPriceForm.service_id || !addPriceForm.species || (!addPriceForm.price_low && !addPriceForm.call_for_quote)) { setAddPriceError(true); return; }
    ...
    if (!error) {
      ...
      setAddPriceError(false); fetchStats();
      if (callReviewVetId === selectedVetId) fetchReviewPrices(callReviewVetId);
    } else {
      alert("Error: " + error.message);
    }
  ...
  const filteredPriceVets = vetPriceSearch.trim() ? vets.filter((v) => v.name.toLowerCase().includes(vetPriceSearch.trim().toLowerCase())) : vets;

  function selectPriceVet(v) {
    setSelectedVetId(v.id); setVetPriceSearch(v.name); setShowVetDropdown(false);
    setEditingPrice(null); setShowAddPrice(false); fetchPricesForVet(v.id);
  }

  function handlePriceSearchChange(e) {
    const val = e.target.value; setVetPriceSearch(val); setShowVetDropdown(true);
    if (!val.trim()) { setSelectedVetId(""); setVetPrices([]); }
  }
      ...
      <div className="row-edit-bg" style={{ marginTop: "10px" }}>
        ...
        <div className="form-grid-3" style={{ marginBottom: "12px" }}>
          <div>
            ...
            <select className="adm-input" value={form.accepting_new_patients === null ? "unknown" : form.accepting_new_patients ? "yes" : "no"}
              onChange={(e) => setForm((p) => ({ ...p, accepting_new_patients: e.target.value === "unknown" ? null : e.target.value === "yes" }))}>
  ...
  function ServiceDropdown({ value, onChange, style }) {
    const grouped = {};
    services.forEach((s) => { const cat = s.category || "Other"; if (!grouped[cat]) grouped[cat] = []; grouped[cat].push(s); });
    const catOrder = ["Exam", "Vaccine", "Dental", "Surgery", "Other"];
    const sorted = [...new Set([...catOrder, ...Object.keys(grouped)])].filter((c) => grouped[c]);
    return (
      <select className="adm-input" value={value} onChange={onChange} style={style}>
  ...
  function IncludesPills({ form, setForm, fields }) {
    return (
      <div style={{ display: "flex", gap: "6px", flexWrap: "nowrap", marginBottom: "10px" }}>
        {[["includes_bloodwork","Bloodwork"],["includes_xrays","X-rays"],["includes_anesthesia","Anesthesia"]].map(([f,l]) => (
          <button key={f} type="button" onClick={() => setForm((prev) => ({ ...prev, [f]: !prev[f] }))}
            style={{ padding:"3px 8px", borderRadius:"20px", fontSize:"11px", fontWeight:"600", cursor:"pointer", border: form[f] ? "none" : "1px solid #ddd", background: form[f] ? "#2d6a4f" : "#f5f5f5", color: form[f] ? "#fff" : "#555", whiteSpace:"nowrap" }}>
            {l}
          </button>
        ))}
      </div>
  ...
  return (
    <>
          ...
          <div className="tabs-scroll" style={{ position: "relative" }}>
            {TABS.map((t) => (
              <button key={t} className={`tab-btn${tab === t ? " active" : ""}`}
                onClick={() => {
                  const doSwitch = () => {
                    setTab(t); setEditingVet(null); setEditingPendingVet(null);
                    setEditingPrice(null); setShowAddPrice(false); setCallReviewEditing(null);
                    setShowInviteForm(false); setInviteError(""); setTeamEditingId(null);
                    setTeamEditName(""); setShowCallbackNotes(false); setCallbackNoteText("");
                    setCallPrices([]); setLockedVetId(null); setLockedVetName("");
                    if (t === "Prices" && selectedVetId) { setPricesLoading(true); fetchPricesForVet(selectedVetId); }
                    fetchAllCallNotes();
                    if (t === "Call Sheet" && callReviewVetId) fetchReviewPrices(callReviewVetId);
                    if (t === "Team") fetchAdminUsers();
                  };
                  if (callPrices.length > 0 && t !== tab && tab === "Call Sheet") {
                    setUnsavedModal({ message: "You have unsaved prices. Switch tabs anyway?", action: doSwitch });
                  } else { doSwitch(); }
                }}>
                {t}
          ...
          <div style={{ padding: "20px" }}>
            ...
            {tab === "Pending Vets" && (
              <div>
                ...
                <input className="adm-input" value={pendingVetSearch} onChange={(e) => setPendingVetSearch(e.target.value)}
                  placeholder="Search by name..." style={{ marginBottom: "14px", maxWidth: "360px" }} />
                ...
                {pendingVets.filter((v) => !pendingVetSearch.trim() || v.name.toLowerCase().includes(pendingVetSearch.toLowerCase())).map((vet) => (
                  <div key={vet.id} className="pending-vet-card">
                    <div className="pv-card-inner" style={{ marginBottom: editingPendingVet === vet.id ? "4px" : "0" }}>
                      <div>
                        {vet.source && <div style={{ marginBottom: "4px" }}><span style={{ fontSize: "11px", background: "#f0f0f0", color: "#666", padding: "1px 7px", borderRadius: "4px" }}>via {vet.source}</span></div>}
                        <div style={{ marginBottom: "2px" }}><span style={{ fontWeight: "700", fontSize: "14px", color: "#111" }}>{vet.name}</span></div>
                        ...
                        {vet.website && <p style={{ margin: "2px 0 0 0", fontSize: "13px", color: "#2d6a4f" }}>{cleanWebsiteUrl(vet.website)}</p>}
                      ...
                      <div className="pv-buttons">
                        ...
                        <button className="adm-btn adm-btn-outline" onClick={() => {
                          if (editingPendingVet === vet.id) { setEditingPendingVet(null); }
                          else { setEditingPendingVet(vet.id); setPendingVetForm({ ...vet }); }
                        }}>{editingPendingVet === vet.id ? "Cancel" : "✏️ Edit"}</button>
                        <button className="adm-btn adm-btn-red" onClick={() => rejectPendingVet(vet.id)}>✕ Reject</button>
                      </div>
                    </div>
                    {editingPendingVet === vet.id && (
                      <PendingVetEditForm form={pendingVetForm} setForm={setPendingVetForm}
                        onApprove={() => approvePendingVet(vet)} onCancel={() => setEditingPendingVet(null)} />
                    )}
                  </div>
                ))}
              </div>
                ...
                {showAddVet && (
                  <div className="row-edit-bg" style={{ marginBottom: "16px" }}>
                    ...
                    <div className="form-grid-4" style={{ marginBottom: "10px" }}>
                      ...
                      <div><label className="field-label">Website</label><input className="adm-input" value={addVetForm.website} onChange={(e) => setAddVetForm({ ...addVetForm, website: e.target.value })} placeholder="e.g. clinicname.com" /></div>
                    </div>
                    <div className="form-grid-4" style={{ marginBottom: "10px" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "4px", marginTop: "2px" }}>
                          {VET_TYPES.map((t) => (
                            <label key={t} style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", cursor: "pointer" }}>
                              <input type="checkbox" checked={(addVetForm.vet_type || []).includes(t)} onChange={(e) => {
                                const cur = addVetForm.vet_type || [];
                                setAddVetForm({ ...addVetForm, vet_type: e.target.checked ? [...cur, t] : cur.filter((x) => x !== t) });
                              }} />{t}
                            </label>
                          ))}
                        </div>
                    ...
                    <div className="form-grid-3" style={{ marginBottom: "10px" }}>
                      <div>
                        <label className="field-label">Accepting Patients</label>
                        <select className="adm-input" value={addVetForm.accepting_new_patients === null ? "unknown" : addVetForm.accepting_new_patients ? "yes" : "no"}
                          onChange={(e) => setAddVetForm({ ...addVetForm, accepting_new_patients: e.target.value === "unknown" ? null : e.target.value === "yes" })}>
                          <option value="unknown">Unknown</option><option value="yes">Yes</option><option value="no">No</option>
                        </select>
                      </div>
                      <div>
                        <label className="field-label">CareCredit</label>
                        <select className="adm-input" value={addVetForm.carecredit ? "yes" : "no"} onChange={(e) => setAddVetForm({ ...addVetForm, carecredit: e.target.value === "yes" })}>
                          <option value="no">No</option><option value="yes">Yes</option>
                        </select>
                      </div>
                        ...
                        <div className="vet-row-inner">
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
                              <span style={{ fontWeight: "600", fontSize: "14px", color: "#111" }}>{vet.name}</span>
                              {vet.internal_notes === "declined_to_share" && <span style={{ fontSize: "11px", background: "#fce4ec", color: "#c62828", padding: "1px 7px", borderRadius: "20px", fontWeight: "600", border: "1px solid #ef9a9a" }}>🚫 Declined</span>}
                            </div>
                        ...
                        <div style={{ padding: "0 14px 14px 14px" }}>
                          <div className="row-edit-bg">
                            ...
                            <div className="form-grid-4" style={{ marginBottom: "10px" }}>
                              ...
                              <div><label className="field-label">Website</label><input className="adm-input" value={vetForm.website || ""} onChange={(e) => setVetForm({ ...vetForm, website: e.target.value })} placeholder="clinicname.com" /></div>
                            </div>
                            <div className="form-grid-4" style={{ marginBottom: "10px" }}>
                              ...
                              <div>
                                ...
                                <div style={{ display: "flex", flexDirection: "column", gap: "4px", marginTop: "2px" }}>
                                  {VET_TYPES.map((t) => (
                                    <label key={t} style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", cursor: "pointer" }}>
                                      <input type="checkbox" checked={(Array.isArray(vetForm.vet_type) ? vetForm.vet_type : [vetForm.vet_type]).includes(t)}
                                        onChange={(e) => {
                                          const cur = Array.isArray(vetForm.vet_type) ? vetForm.vet_type : [vetForm.vet_type];
                                          setVetForm({ ...vetForm, vet_type: e.target.checked ? [...cur, t] : cur.filter((x) => x !== t) });
                                        }} />{t}
                                    </label>
                                  ))}
                                </div>
                            ...
                            <div className="form-grid-3" style={{ marginBottom: "10px" }}>
                              <div>
                                <label className="field-label">Accepting Patients</label>
                                <select className="adm-input" value={vetForm.accepting_new_patients == null ? "unknown" : vetForm.accepting_new_patients ? "yes" : "no"}
                                  onChange={(e) => setVetForm({ ...vetForm, accepting_new_patients: e.target.value === "unknown" ? null : e.target.value === "yes" })}>
                                  <option value="unknown">Unknown</option><option value="yes">Yes</option><option value="no">No</option>
                                </select>
                              </div>
                              <div>
                                <label className="field-label">CareCredit</label>
                                <select className="adm-input" value={vetForm.carecredit ? "yes" : "no"} onChange={(e) => setVetForm({ ...vetForm, carecredit: e.target.value === "yes" })}>
                                  <option value="no">No</option><option value="yes">Yes</option>
                                </select>
                              </div>
            ...
            {tab === "Prices" && (
              <div>
                <div className="section-header">
                  <h2 style={{ margin: 0, fontSize: "1rem", color: "#111" }}>Prices</h2>
                </div>
                <div style={{ marginBottom: "20px", maxWidth: "400px" }}>
                  <label className="field-label">Search Vet</label>
                  <div className="price-search-wrap">
                    <div style={{ position: "relative" }}>
                      <input ref={priceSearchRef} className="adm-input" style={{ paddingRight: vetPriceSearch ? "32px" : "12px" }}
                        value={vetPriceSearch} onChange={handlePriceSearchChange} onFocus={() => setShowVetDropdown(true)}
                        onBlur={() => { setTimeout(() => setShowVetDropdown(false), 150); }}
                        placeholder="Click to browse or type to filter..." autoComplete="off" />
                      {vetPriceSearch && (
                        <button type="button" onClick={() => { setVetPriceSearch(""); setSelectedVetId(""); setVetPrices([]); setShowVetDropdown(false); setEditingPrice(null); setShowAddPrice(false); }}
                          style={{ position: "absolute", right: "10px", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", fontSize: "16px", color: "#aaa", lineHeight: 1, padding: 0 }}>✕</button>
                      )}
                    </div>
                    {showVetDropdown && (
                      <div className="price-search-dropdown">
                        {filteredPriceVets.length === 0 && <div style={{ padding: "10px 12px", fontSize: "13px", color: "#888" }}>No vets found</div>}
                        {filteredPriceVets.slice(0, 50).map((v) => (
                          <div key={v.id} className={`price-search-item${selectedVetId === v.id ? " selected" : ""}`} onMouseDown={() => selectPriceVet(v)}>
                            {v.name}{v.city ? <span style={{ color: "#888", fontSize: "13px" }}> — {v.city}</span> : ""}
                          </div>
                        ))}
                      </div>
                    )}
                  </div>
                  {selectedVetId && <p style={{ margin: "6px 0 0 0", fontSize: "13px", color: "#2d6a4f" }}>✓ Showing prices for <strong>{vetPriceSearch}</strong></p>}
                </div>
                      ...
                      <div className="row-edit-bg" style={{ marginBottom: "14px" }}>
                        ...
                        <div className="form-grid-4" style={{ marginBottom: "10px" }}>
                          <div>
                            ...
                            <ServiceDropdown value={addPriceForm.service_id} onChange={(e) => { setAddPriceError(false); setAddPriceForm({ ...addPriceForm, service_id: e.target.value }); }} style={addPriceError && !addPriceForm.service_id ? { borderColor: "#c62828", borderWidth: "2px" } : {}} />
                          </div>
                          ...
                          <div><label className="field-label">Price Low *</label><input className="adm-input" type="number" value={addPriceForm.price_low} onChange={(e) => { setAddPriceError(false); setAddPriceForm({ ...addPriceForm, price_low: e.target.value }); }} placeholder="e.g. 65" style={addPriceError && !addPriceForm.price_low && !addPriceForm.call_for_quote ? { borderColor: "#c62828", borderWidth: "2px" } : {}} /></div>
                        ...
                        <div className="form-grid-2" style={{ marginBottom: "14px" }}>
                          <div>
                            ...
                            <select className="adm-input" value={addPriceForm.species === "other" || (addPriceForm.species && !["dog","cat","rabbit","bird","other",""].includes(addPriceForm.species)) ? "other" : addPriceForm.species || ""}
                              onChange={(e) => { setAddPriceError(false); setAddPriceForm({ ...addPriceForm, species: e.target.value, species_other: "" }); }}
                              style={addPriceError && !addPriceForm.species ? { borderColor: "#c62828", borderWidth: "2px" } : {}}>
                            ...
                            {(addPriceForm.species === "other" || (addPriceForm.species && !["dog","cat","rabbit","bird","other",""].includes(addPriceForm.species))) && (
                              <input className="adm-input" style={{ marginTop: "8px" }} value={addPriceForm.species_other || ""} onChange={(e) => setAddPriceForm({ ...addPriceForm, species: "other", species_other: e.target.value })} placeholder="e.g. Guinea pig, snake..." />
                            )}
                          </div>
                          <div>
                            <label className="field-label" style={{ display: "block", marginBottom: "8px" }}>Includes</label>
                            <IncludesPills form={addPriceForm} setForm={setAddPriceForm} />
                        ...
                        {addPriceError && (
                          <div style={{ background: "#fff0f0", border: "1px solid #ffcdd2", borderRadius: "8px", padding: "10px 14px", marginBottom: "10px" }}>
                            <p style={{ margin: 0, fontSize: "13px", color: "#c62828", fontWeight: "600" }}>⚠️ Please fill in all required fields:</p>
                            <ul style={{ margin: "6px 0 0 0", paddingLeft: "18px", fontSize: "13px", color: "#c62828" }}>
                              {!addPriceForm.service_id && <li>Service is required</li>}
                              {!addPriceForm.species && <li>Species is required</li>}
                              {!addPriceForm.price_low && !addPriceForm.call_for_quote && <li>Price or "Call for quote" is required</li>}
                            </ul>
                          </div>
                        )}
                        ...
                        <div key={p.id}>
                          <div className="price-row-wrap">
                            <div className="price-row-info">
                              <div style={{ fontWeight: "600", fontSize: "14px", color: "#111", marginBottom: "6px" }}>{p.services?.name || "—"}</div>
                              <div style={{ marginBottom: "8px" }}>
                                <span style={{ fontSize: "14px", color: "#2d6a4f", fontWeight: "600" }}>{formatPrice(p.price_low, p.price_high, p.price_type)}</span>
                                <span style={{ marginLeft: "8px", fontSize: "12px", color: "#888" }}>({p.price_type})</span>
                              </div>
                              {p.species && <div style={{ marginBottom: "6px" }}><span style={{ fontSize: "12px", background: "#f0f0f0", color: "#555", padding: "2px 8px", borderRadius: "4px", textTransform: "capitalize" }}>{p.species}</span></div>}
                              {(p.includes_bloodwork || p.includes_xrays || p.includes_anesthesia || p.call_for_quote) && (
                              ...
                              {p.notes && <p style={{ margin: "6px 0 0 0", fontSize: "12px", color: "#888", fontStyle: "italic" }}>{p.notes}</p>}
                            </div>
                          ...
                          {editingPrice === p.id && (
                            <div style={{ padding: "0 12px 14px 12px" }}>
                              <div className="row-edit-bg" style={{ margin: 0 }}>
                                <div className="form-grid-4" style={{ marginBottom: "10px" }}>
                                  <div><label className="field-label">Service</label><ServiceDropdown value={priceForm.service_id || ""} onChange={(e) => setPriceForm({ ...priceForm, service_id: e.target.value })} /></div>
                                ...
                                <div className="form-grid-2" style={{ marginBottom: "14px" }}>
                                  <div>
                                    <label className="field-label">Species</label>
                                    <select className="adm-input" value={priceForm.species === "other" || (priceForm.species && !["dog","cat","rabbit","bird","other",""].includes(priceForm.species)) ? "other" : priceForm.species || ""}
                                      onChange={(e) => { setEditPriceError(false); setPriceForm({ ...priceForm, species: e.target.value, species_other: "" }); }}
                                      style={editPriceError && !priceForm.species ? { borderColor: "#c62828", borderWidth: "2px" } : {}}>
                                      <option value="">— Select —</option><option value="dog">Dog</option><option value="cat">Cat</option><option value="rabbit">Rabbit</option><option value="bird">Bird</option><option value="other">Other...</option>
                                    </select>
                                    {(priceForm.species === "other" || (priceForm.species && !["dog","cat","rabbit","bird","other",""].includes(priceForm.species))) && (
                                      <input className="adm-input" style={{ marginTop: "8px" }} value={priceForm.species_other || ""} onChange={(e) => setPriceForm({ ...priceForm, species: "other", species_other: e.target.value })} placeholder="e.g. Guinea pig, snake..." />
                                    )}
                                  </div>
                                  <div>
                                    <label className="field-label" style={{ display: "block", marginBottom: "8px" }}>Includes</label>
                                    <IncludesPills form={priceForm} setForm={setPriceForm} />
                                    <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", cursor: "pointer" }}>
                                ...
                                <div style={{ marginBottom: "14px", marginTop: "6px" }}><label className="field-label">Notes</label><textarea className="adm-input" rows={3} style={{ width: "100%", resize: "vertical", height: "auto" }} value={priceForm.notes || ""} onChange={(e) => setPriceForm({ ...priceForm, notes: e.target.value })} placeholder="Notes about this price..." /></div>
                                {editPriceError && (
                                  <div style={{ background: "#fff0f0", border: "1px solid #ffcdd2", borderRadius: "8px", padding: "10px 14px", marginBottom: "10px" }}>
                                    <p style={{ margin: 0, fontSize: "13px", color: "#c62828", fontWeight: "600" }}>⚠️ Please fill in all required fields:</p>
                                    <ul style={{ margin: "6px 0 0 0", paddingLeft: "18px", fontSize: "13px", color: "#c62828" }}>
                                      {!priceForm.service_id && <li>Service is required</li>}
                                      {!priceForm.species && <li>Species is required</li>}
                                      {!priceForm.price_low && !priceForm.call_for_quote && <li>Price or "Call for quote" is required</li>}
                                    </ul>
                                  </div>
                                )}
                                <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end" }}>
                                  <button className="adm-btn adm-btn-gray" onClick={() => { setEditingPrice(null); setEditPriceError(false); }}>Cancel</button>
                ...
                {unverifiedPrices.length > 0 && (
                  <div style={{ marginTop: "24px" }}>
                    <div className="section-header" style={{ marginBottom: "12px" }}>
                      <div>
                        <h2 style={{ margin: 0, fontSize: "1rem", color: "#111" }}>🤖 AI-Found Prices
            ...
            {tab === "Call Sheet" && (() => {
              ...
              return (
                <div>
                  ...
                  {!callQueueLoading && activeQueue.length === 0 && <div style={{ textAlign: "center", padding: "48px 20px" }}><p style={{ fontSize: "32px", margin: "0 0 8px 0" }}>🎉</p><p style={{ color: "#bbb", fontSize: "14px" }}>All vets have been called!</p></div>}
                  ...
                  {callLog.length > 0 && (
                    <div style={{ background: "#f9f9f9", border: "1px solid #eee", borderRadius: "8px", padding: "8px 12px", marginBottom: "14px" }}>
                      ...
                      {callLog.map((entry, i) => <p key={i} style={{ margin: "2px 0", fontSize: "13px", color: "#555" }}>✅ <strong>{entry.name}</strong> — {entry.count} price{entry.count !== 1 ? "s" : ""} saved</p>)}
                    </div>
                  )}
                      ...
                      <div>
                        <div className="call-sheet-header" style={{ marginBottom: "16px" }}>
                          ...
                          <div style={{ display: "inline-flex", borderRadius: "6px", border: "1px solid #2d6a4f", overflow: "hidden", flexShrink: 0 }}>
                            <button type="button" onClick={() => { setShowAllVets(false); setCallIndex(0); setCallPrices([]); setCallReviewPrices([]); setCallReviewVetId(null); setCallSaved(false); }}
                              style={{ padding: "6px 14px", fontSize: "12px", fontWeight: "600", cursor: "pointer", border: "none", background: !showAllVets ? "#2d6a4f" : "#fff", color: !showAllVets ? "#fff" : "#2d6a4f" }}>Unpriced</button>
                            <button type="button" onClick={() => { setShowAllVets(true); setCallIndex(0); setCallPrices([]); setCallReviewPrices([]); setCallReviewVetId(null); setCallSaved(false); }}
                              style={{ padding: "6px 14px", fontSize: "12px", fontWeight: "600", cursor: "pointer", border: "none", borderLeft: "1px solid #2d6a4f", background: showAllVets ? "#2d6a4f" : "#fff", color: showAllVets ? "#fff" : "#2d6a4f" }}>All Vets</button>
                          </div>
                          <div style={{ display: "flex", gap: "8px" }}>
                            <button className="adm-btn adm-btn-gray" onClick={() => {
                              if (callPrices.length > 0) { setUnsavedModal({ message: "You have unsaved prices. Refresh anyway?", action: () => { fetchCallQueue(); setCallIndex(0); setCallPrices([]); setCallReviewPrices([]); setCallReviewVetId(null); setCallSaved(false); setCallReviewEditing(null); setLockedVetId(null); setLockedVetName(""); } }); return; }
                              fetchCallQueue(); setCallIndex(0); setCallPrices([]); setCallReviewPrices([]); setCallReviewVetId(null); setCallSaved(false); setCallReviewEditing(null); setLockedVetId(null); setLockedVetName("");
                            }}>↺ Refresh</button>
                            <button className="adm-btn adm-btn-gray" onClick={() => {
                              if (callPrices.length > 0) { setUnsavedModal({ message: "You have unsaved prices. Go back anyway?", action: () => { setCallIndex((i) => Math.max(0, i - 1)); setCallPrices([]); setLockedVetId(null); setLockedVetName(""); } }); return; }
                              setCallIndex((i) => Math.max(0, i - 1)); setCallPrices([]); setLockedVetId(null); setLockedVetName("");
                            }} disabled={callIndex === 0}>← Prev</button>
                            <button className="adm-btn adm-btn-gray" onClick={() => {
                              if (callPrices.length > 0) { setUnsavedModal({ message: "You have unsaved prices. Skip this vet anyway?", action: () => { setCallIndex((i) => i + 1); setCallPrices([]); setLockedVetId(null); setLockedVetName(""); } }); return; }
                              setCallIndex((i) => i + 1); setCallPrices([]); setLockedVetId(null); setLockedVetName("");
                            }}>Skip →</button>
                          </div>
                        </div>

                        {/* Call sheet search */}
                        <div style={{ marginBottom: "14px", position: "relative", maxWidth: "400px" }}>
                          <input className="adm-input" value={callSheetSearch} onChange={(e) => setCallSheetSearch(e.target.value)} placeholder="Search vet by name to jump to them..." />
                          {callSheetSearch.trim() && (
                            <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, background: "#fff", border: "1px solid #e8e8e8", borderRadius: "8px", maxHeight: "200px", overflowY: "auto", zIndex: 100, boxShadow: "0 4px 16px rgba(0,0,0,0.10)" }}>
                              {activeQueue.filter((v) => v.name.toLowerCase().includes(callSheetSearch.toLowerCase())).slice(0, 8).map((v) => {
                                const realIdx = activeQueue.findIndex((q) => q.id === v.id && q._source === v._source);
                                return (
                                  <div key={v.id + v._source} style={{ padding: "9px 12px", cursor: "pointer", fontSize: "13px", borderBottom: "1px solid #f5f5f5" }}
                                    onMouseDown={() => {
                                      if (callPrices.length > 0) { setUnsavedModal({ message: "You have unsaved prices. Jump to another vet anyway?", action: () => { setCallIndex(realIdx); setCallPrices([]); setLockedVetId(null); setLockedVetName(""); setCallSheetSearch(""); } }); return; }
                                      setCallIndex(realIdx); setCallPrices([]); setLockedVetId(null); setLockedVetName(""); setCallSheetSearch("");
                                    }}>
                                    {v.name}<span style={{ fontSize: "11px", color: "#aaa", marginLeft: "8px" }}>#{realIdx + 1}</span>
                                  </div>
                                );
                              })}
                              {activeQueue.filter((v) => v.name.toLowerCase().includes(callSheetSearch.toLowerCase())).length === 0 && <div style={{ padding: "9px 12px", fontSize: "13px", color: "#aaa" }}>No vets found</div>}
                            </div>
                          )}
                        </div>
                        ...
                        <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: "12px", padding: "20px", marginBottom: "16px" }}>
                          <div style={{ display: "flex", gap: "6px", marginBottom: "6px", flexWrap: "wrap" }}>
                            <span style={{ fontSize: "11px", background: vet._source === "pending" ? "#fff8e1" : "#e8f5e9", color: vet._source === "pending" ? "#e65100" : "#2d6a4f", padding: "2px 8px", borderRadius: "20px", fontWeight: "600" }}>{vet._source === "pending" ? "New" : "Active"}</span>
                            {vet._hasPrices && <span style={{ fontSize: "11px", background: "#e8f5e9", color: "#2d6a4f", padding: "2px 8px", borderRadius: "20px", fontWeight: "600", border: "1px solid #c8e6c9" }}>✓ Priced</span>}
                            {vet._declined && <span style={{ fontSize: "11px", background: "#fce4ec", color: "#c62828", padding: "2px 8px", borderRadius: "20px", fontWeight: "600", border: "1px solid #ef9a9a" }}>🚫 Declined</span>}
                          </div>
                          ...
                          {((vet.city && vet.city.length > 2) || (vet.neighborhood && vet.neighborhood.length > 2)) ? (
                            <p style={{ margin: "0 0 8px 0", fontSize: "14px", color: "#555" }}>
                              {[(vet.city && vet.city.length > 2 ? vet.city : null) || (vet.neighborhood && vet.neighborhood.length > 2 ? vet.neighborhood : null), vet.state, vet.zip_code].filter(Boolean).join(", ")}
                            </p>
                          ) : (
                            <div style={{ margin: "0 0 8px 0" }}>
                              ...
                              <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                                <input className="adm-input" style={{ maxWidth: "180px", fontSize: "13px", padding: "4px 8px" }} placeholder="Enter city..." defaultValue=""
                                  onBlur={async (e) => {
                                    const city = e.target.value.trim(); if (!city) return;
                          ...
                          {vet.website && (() => {
                            try {
                              const cleanUrl = cleanWebsiteUrl(vet.website);
                              return <a href={vet.website.startsWith("http") ? vet.website : `https://${cleanUrl}`} target="_blank" rel="noreferrer" style={{ fontSize: "13px", color: "#2d6a4f", display: "block", marginBottom: "8px" }}>{cleanUrl}</a>;
                            } catch { return <a href={vet.website} target="_blank" rel="noreferrer" style={{ fontSize: "13px", color: "#2d6a4f", display: "block", marginBottom: "8px" }}>{vet.website}</a>; }
                          })()}
                          {vet.phone && <a href={`tel:${vet.phone}`} style={{ display: "inline-flex", alignItems: "center", background: "#2d6a4f", color: "#fff", padding: "5px 10px", borderRadius: "6px", textDecoration: "none", fontWeight: "600", fontSize: "13px", marginBottom: "12px" }}>{vet.phone}</a>}

                          <div className="form-grid-2" style={{ marginBottom: "14px", marginTop: "16px" }}>
                            <div>
                              <label className="field-label">Accepting New Patients</label>
                              <select className="adm-input" value={vet.accepting_new_patients === true ? "yes" : vet.accepting_new_patients === false ? "no" : ""}
                                onChange={async (e) => {
                                  const dbVal = e.target.value === "yes" ? true : e.target.value === "no" ? false : null;
                                  const table = vet._source === "pending" ? "pending_vets" : "vets";
                                  await supabase.from(table).update({ accepting_new_patients: dbVal }).eq("id", vet.id);
                                  setCallQueue((prev) => prev.map((v, i) => i === callIndex ? { ...v, accepting_new_patients: dbVal } : v));
                                  setFullCallQueue((prev) => prev.map((v, i) => i === callIndex ? { ...v, accepting_new_patients: dbVal } : v));
                                }}>
                            ...
                            <div>
                              <label className="field-label">CareCredit</label>
                              <select className="adm-input" value={vet.carecredit === true ? "yes" : vet.carecredit === false ? "no" : ""}
                                onChange={async (e) => {
                                  const dbVal = e.target.value === "yes" ? true : e.target.value === "no" ? false : null;
                                  const table = vet._source === "pending" ? "pending_vets" : "vets";
                                  await supabase.from(table).update({ carecredit: dbVal }).eq("id", vet.id);
                                  setCallQueue((prev) => prev.map((v, i) => i === callIndex ? { ...v, carecredit: dbVal } : v));
                                  setFullCallQueue((prev) => prev.map((v, i) => i === callIndex ? { ...v, carecredit: dbVal } : v));
                                }}>
                                <option value="">— Select —</option><option value="yes">Yes</option><option value="no">No</option>
                              </select>
                            </div>
                          ...
                          <div style={{ borderTop: "1px solid #f0f0f0", paddingTop: "14px", marginBottom: "16px" }}>
                            ...
                            <div className="call-no-prices-btns">
                              <button className="adm-btn adm-btn-gray" onClick={() => markCallStatus(vet, "declined")}>🚫 Declined to share</button>
                              <button className="adm-btn adm-btn-gray" onClick={() => { setNotesVetId(vet.id); setNotesVetName(vet.name); fetchCallNotes(vet.id); setShowCallbackNotes(true); setCallPrices([]); }}>🕐 Call back later</button>
                            </div>
                            {showCallbackNotes && notesVetId === vet.id && (
                              <div style={{ marginTop: "14px", background: "#f9f9f9", border: "1px solid #e8e8e8", borderRadius: "10px", padding: "16px" }}>
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "12px", marginBottom: "14px" }}>
                                  <p style={{ margin: 0, fontSize: "14px", fontWeight: "700", color: "#111" }}>Notes for {vet.name}</p>
                                  <button onClick={() => setShowCallbackNotes(false)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "18px", color: "#aaa", lineHeight: 1, padding: "0 2px", flexShrink: 0 }}>✕</button>
                                </div>
                                <div style={{ marginBottom: "16px" }}>
                                  <textarea className="adm-input" rows={3} style={{ width: "100%", resize: "vertical", height: "auto", marginBottom: "8px" }} value={callbackNoteText} onChange={(e) => setCallbackNoteText(e.target.value)} placeholder="Add a note about this vet..." />
                                  <div style={{ display: "flex", justifyContent: "flex-end" }}>
                                    <button className="adm-btn adm-btn-green" onClick={() => saveCallNote(vet.id, vet.name, callbackNoteText)}>Save Note</button>
                                  </div>
                                </div>
                                {callNotes.length === 0 && <p style={{ fontSize: "13px", color: "#aaa", fontStyle: "italic", margin: 0 }}>No notes yet.</p>}
                                {callNotes.map((n) => (
                                  <div key={n.id} style={{ borderTop: "1px solid #ebebeb", paddingTop: "12px", marginTop: "12px" }}>
                                    {editingNoteId === n.id ? (
                                      <div>
                                        <textarea className="adm-input" rows={3} style={{ width: "100%", resize: "vertical", height: "auto", marginBottom: "8px" }} value={editingNoteText} onChange={(e) => setEditingNoteText(e.target.value)} />
                                        <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px" }}>
                                          <button className="adm-btn adm-btn-gray" onClick={() => { setEditingNoteId(null); setEditingNoteText(""); }}>Cancel</button>
                                          <button className="adm-btn adm-btn-green" onClick={() => updateCallNote(n.id, editingNoteText)}>Save Changes</button>
                                        </div>
                                      </div>
                                    ) : deletingNoteId === n.id ? (
                                      <div style={{ background: "#fff0f0", border: "1px solid #ffcdd2", borderRadius: "6px", padding: "12px" }}>
                                        <p style={{ margin: "0 0 10px 0", fontSize: "13px", color: "#c62828", fontWeight: "600" }}>Delete this note?</p>
                                        <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px" }}>
                                          <button className="adm-btn adm-btn-gray" onClick={() => setDeletingNoteId(null)}>Cancel</button>
                                          <button className="adm-btn adm-btn-red" onClick={() => deleteCallNote(n.id)}>Yes, Delete</button>
                                        </div>
                                      </div>
                                    ) : (
                                      <div>
                                        <p style={{ margin: "0 0 6px 0", fontSize: "11px", color: "#aaa" }}>{formatLogDate(n.created_at)}{n.updated_at !== n.created_at ? " · edited" : ""}</p>
                                        <p style={{ margin: "0 0 10px 0", fontSize: "13px", color: "#333", lineHeight: "1.5" }}>{n.note}</p>
                                        <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px" }}>
                                          <button className="adm-btn adm-btn-outline" onClick={() => { setEditingNoteId(n.id); setEditingNoteText(n.note); }}>Edit</button>
                                          <button className="adm-btn adm-btn-red" onClick={() => setDeletingNoteId(n.id)}>Delete</button>
                                        </div>
                                      </div>
                                    )}
                                  </div>
                          ...
                          <div>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
                              ...
                              <button className="adm-btn adm-btn-outline" onClick={() => { addCallPriceRow(vet); setShowCallbackNotes(false); setCallbackNoteText(""); }}>+ Add Price</button>
                            </div>
                            {callPrices.length === 0 && <p style={{ color: "#bbb", fontSize: "13px", fontStyle: "italic", margin: "0 0 12px 0" }}>No prices added yet — click "+ Add Price" for each service they quote you.</p>}
                            {callPrices.map((p, i) => (
                              <div key={i} className="row-edit-bg" style={{ marginBottom: "20px" }}>
                                <div style={{ marginBottom: "10px" }}>
                                  <label className="field-label">Service</label>
                                  <ServiceDropdown value={p.service_id} onChange={(e) => { setCallSpeciesError(false); updateCallPrice(i, "service_id", e.target.value); }} style={callSpeciesError && !p.service_id ? { borderColor: "#c62828", borderWidth: "2px" } : {}} />
                                </div>
                                {(() => {
                                  const svc = services.find((s) => s.id === parseInt(p.service_id) || s.id === p.service_id);
                                  return svc?.name?.toLowerCase().includes("package") && svc?.category === "Vaccine";
                                })() && (
                                  <div style={{ marginBottom: "10px" }}>
                                    <label className="field-label" style={{ display: "block", marginBottom: "6px" }}>Vaccines Included</label>
                                    <input className="adm-input" value={p.vaccines_included || ""} onChange={(e) => updateCallPrice(i, "vaccines_included", e.target.value)} placeholder="e.g. Rabies, DHPP, Bordetella..." />
                                    <p style={{ margin: "4px 0 0 0", fontSize: "11px", color: "#aaa" }}>This will display publicly on the vet profile</p>
                                  </div>
                                )}
                                <div className="form-grid-3" style={{ marginBottom: "14px" }}>
                                  ...
                                  <div><label className="field-label">Price Low</label><input className="adm-input" type="number" value={p.price_low} onChange={(e) => { setCallSpeciesError(false); updateCallPrice(i, "price_low", e.target.value); }} placeholder="e.g. 65" style={callSpeciesError && !p.price_low && !p.call_for_quote ? { borderColor: "#c62828", borderWidth: "2px" } : {}} /></div>
                                ...
                                <div className="form-grid-2" style={{ marginBottom: "14px" }}>
                                  <div>
                                    ...
                                    <select className="adm-input" value={p.species === "other" || (p.species && !["dog","cat","rabbit","bird","other",""].includes(p.species)) ? "other" : p.species || ""}
                                      onChange={(e) => { setCallSpeciesError(false); setCallPrices((prev) => prev.map((row, idx) => idx === i ? { ...row, species: e.target.value, speciesOther: e.target.value !== "other" ? "" : row.speciesOther } : row)); }}
                                      style={callSpeciesError && !p.species ? { borderColor: "#c62828", borderWidth: "2px" } : {}}>
                                      <option value="">— Select —</option><option value="dog">Dog</option><option value="cat">Cat</option><option value="rabbit">Rabbit</option><option value="bird">Bird</option><option value="other">Other...</option>
                                    </select>
                                    {(p.species === "other" || (p.species && !["dog","cat","rabbit","bird","other",""].includes(p.species))) && (
                                      <input className="adm-input" style={{ marginTop: "8px" }} value={p.speciesOther || ""} onChange={(e) => setCallPrices((prev) => prev.map((row, idx) => idx === i ? { ...row, species: "other", speciesOther: e.target.value } : row))} placeholder="e.g. Guinea pig, bird..." />
                                    )}
                                  </div>
                                  <div>
                                    <label className="field-label" style={{ display: "block", marginBottom: "8px" }}>Includes</label>
                                    <div style={{ display: "flex", gap: "6px", flexWrap: "nowrap" }}>
                                      {[["includes_bloodwork","Bloodwork"],["includes_xrays","X-rays"],["includes_anesthesia","Anesthesia"]].map(([field, label]) => (
                                        <button key={field} type="button" onClick={() => setCallPrices((prev) => prev.map((row, idx) => idx === i ? { ...row, [field]: !row[field] } : row))}
                                          style={{ padding: "3px 8px", borderRadius: "20px", fontSize: "11px", fontWeight: "600", cursor: "pointer", border: p[field] ? "none" : "1px solid #ddd", background: p[field] ? "#2d6a4f" : "#f5f5f5", color: p[field] ? "#fff" : "#555", whiteSpace: "nowrap" }}>
                                          {label}
                                        </button>
                                      ))}
                                    </div>
                              ...
                              <div style={{ marginTop: "8px" }}>
                                {callSpeciesError && (
                                  <div style={{ background: "#fff0f0", border: "1px solid #ffcdd2", borderRadius: "8px", padding: "10px 14px", marginBottom: "10px" }}>
                                    ...
                                    <ul style={{ margin: "6px 0 0 0", paddingLeft: "18px", fontSize: "13px", color: "#c62828" }}>
                                      {callPrices.map((p, idx) => { const missing = []; if (!p.service_id) missing.push("Service"); if (!p.species) missing.push("Species"); if (!p.price_low && !p.call_for_quote) missing.push("Price"); return missing.length > 0 ? <li key={idx}>Row {idx + 1}: {missing.join(", ")}</li> : null; })}
                                    </ul>
                                  </div>
                                )}
                                {lockedVetId && lockedVetId !== vet.id && <p style={{ margin: "0 0 8px 0", fontSize: "12px", color: "#e65100", fontWeight: "600" }}>⚠️ Saving prices for {lockedVetName}</p>}
                                <button className="adm-btn adm-btn-green" style={{ padding: "10px 24px", fontSize: "14px", width: "100%" }} onClick={() => { setCallSpeciesError(false); saveCallPrices(vet); }} disabled={callSaving}>{callSaving ? "Saving..." : "✅ Save Prices"}</button>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        {callReviewPrices.length > 0 && (
                          <div style={{ background: "#fff", border: "2px solid #2d6a4f", borderRadius: "12px", padding: "20px", marginTop: "16px" }}>
                            ...
                            {callReviewPrices.map((row, i) => {
                              ...
                              return (
                                <div key={row.id || i} style={{ borderTop: "1px solid #f0f0f0", paddingTop: "12px", marginTop: "12px" }}>
                                  {!isEditing ? (
                                    <div>
                                      <p style={{ margin: "0 0 6px 0", fontSize: "14px", fontWeight: "600", color: "#111" }}>{svc?.name || "Unknown service"}</p>
                                      <p style={{ margin: "0 0 8px 0", fontSize: "14px", color: "#2d6a4f", fontWeight: "600" }}>
                                        ...
                                        {row.price_type && row.price_type !== "exact" && <span style={{ fontSize: "12px", color: "#888", marginLeft: "6px" }}>({row.price_type})</span>}
                                      </p>
                                      {row.species && <div style={{ marginBottom: "8px" }}><span style={{ fontSize: "12px", background: "#f0f0f0", color: "#555", padding: "2px 8px", borderRadius: "4px", textTransform: "capitalize" }}>{row.species}</span></div>}
                                      {(row.includes_bloodwork === true || row.includes_xrays === true || row.includes_anesthesia === true) && (
                                        <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "8px" }}>
                                          {row.includes_bloodwork === true && <span style={{ fontSize: "12px", background: "#e8f5e9", color: "#2d6a4f", padding: "2px 8px", borderRadius: "4px" }}>+ bloodwork</span>}
                                          {row.includes_xrays === true && <span style={{ fontSize: "12px", background: "#e8f5e9", color: "#2d6a4f", padding: "2px 8px", borderRadius: "4px" }}>+ x-rays</span>}
                                          {row.includes_anesthesia === true && <span style={{ fontSize: "12px", background: "#e8f5e9", color: "#2d6a4f", padding: "2px 8px", borderRadius: "4px" }}>+ anesthesia</span>}
                                        </div>
                                      )}
                                      {row.notes && <p style={{ margin: "0 0 10px 0", fontSize: "13px", color: "#888", fontStyle: "italic" }}>{row.notes}</p>}
                                    ...
                                    <div style={{ background: "#f9f9f9", borderRadius: "8px", padding: "16px" }}>
                                      <div className="form-grid-4" style={{ marginBottom: "14px" }}>
                                        <div><label className="field-label">Service</label><ServiceDropdown value={row.service_id} onChange={(e) => { const u = [...callReviewPrices]; u[i] = { ...u[i], service_id: e.target.value }; setCallReviewPrices(u); }} /></div>
                                      ...
                                      <div className="form-grid-2" style={{ marginBottom: "14px" }}>
                                        <div>
                                          <label className="field-label">Species *</label>
                                          <select className="adm-input" value={row.species === "other" || (row.species && !["dog","cat","rabbit","bird","other",""].includes(row.species)) ? "other" : row.species || ""}
                                            onChange={(e) => { const u = [...callReviewPrices]; u[i] = { ...u[i], species: e.target.value, speciesOther: "" }; setCallReviewPrices(u); }}>
                                            <option value="">— Select —</option><option value="dog">Dog</option><option value="cat">Cat</option><option value="rabbit">Rabbit</option><option value="bird">Bird</option><option value="other">Other...</option>
                                          </select>
                                          {(row.species === "other" || (row.species && !["dog","cat","rabbit","bird","other",""].includes(row.species))) && (
                                            <input className="adm-input" style={{ marginTop: "8px" }} value={row.speciesOther || ""} onChange={(e) => { const u = [...callReviewPrices]; u[i] = { ...u[i], species: "other", speciesOther: e.target.value }; setCallReviewPrices(u); }} placeholder="e.g. Guinea pig, snake..." />
                                          )}
                                        </div>
                                        <div>
                                          <label className="field-label">Includes</label>
                                          <div style={{ display: "flex", gap: "6px", flexWrap: "nowrap", paddingTop: "2px" }}>
                                            {[["includes_bloodwork","Bloodwork"],["includes_xrays","X-rays"],["includes_anesthesia","Anesthesia"]].map(([field, label]) => (
                                              <button key={field} type="button" onClick={() => { const u = [...callReviewPrices]; u[i] = { ...u[i], [field]: !u[i][field] }; setCallReviewPrices(u); }}
                                                style={{ padding: "3px 8px", borderRadius: "20px", fontSize: "11px", fontWeight: "600", cursor: "pointer", border: row[field] ? "none" : "1px solid #ddd", background: row[field] ? "#2d6a4f" : "#f5f5f5", color: row[field] ? "#fff" : "#555", whiteSpace: "nowrap" }}>
                                                {label}
                                              </button>
                                            ))}
                                          </div>
                                      ...
                                      <div style={{ marginBottom: "14px" }}><label className="field-label">Notes</label><textarea className="adm-input" rows={3} style={{ width: "100%", resize: "vertical", height: "auto" }} value={row.notes || ""} onChange={(e) => { const u = [...callReviewPrices]; u[i] = { ...u[i], notes: e.target.value }; setCallReviewPrices(u); }} placeholder="Notes about this price..." /></div>
                                      <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end" }}>
                            ...
                            <div style={{ marginTop: "20px", paddingTop: "16px", borderTop: "1px solid #f0f0f0" }}>
                              ...
                              <button className="adm-btn adm-btn-green" style={{ width: "100%", padding: "12px", fontSize: "14px" }} onClick={advanceFromReview}>Next Vet →</button>
                            </div>
                          </div>
                        )}
                      </div>
            ...
            {tab === "Users" && (
              <div>
                ...
                <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: "8px", overflow: "hidden" }}>
                  <div className="users-table-header"><span>Name</span><span>Zip</span><span>Profile</span><span>Joined</span></div>
                  ...
                  {filteredUsers.map((u) => (
                    <div key={u.id} className="users-table-row">
                      <div className="users-col-name">
                        ...
                        {u.bio && <p className="bio-desktop" style={{ margin: "3px 0 0 0", fontSize: "13px", color: "#888" }}>{u.bio}</p>}
                      </div>
                      ...
                      <div className="users-mobile-detail">
                        {u.bio && <div style={{ marginBottom: "6px" }}><p style={{ margin: "0 0 2px 0", color: "#aaa", fontWeight: "700", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.4px" }}>Bio:</p><p style={{ margin: 0, fontSize: "13px", color: "#666" }}>{u.bio}</p></div>}
            ...
            {tab === "Team" && (
              <div>
                <div className="section-header">
                  <div>
                    <h2 style={{ margin: "0 0 2px 0", fontSize: "1rem", color: "#111" }}>Team</h2>
                    <p style={{ margin: 0, fontSize: "13px", color: "#888" }}>Manage admin access and permissions</p>
                  </div>
                  {adminUsers.find((a) => a.email === currentUserEmail)?.can_manage_team && (
                    <button className="adm-btn adm-btn-green" onClick={() => { setShowInviteForm((v) => !v); setInviteError(""); setTeamEditingId(null); setTeamEditName(""); }}>{showInviteForm ? "Cancel" : "+ Invite Person"}</button>
                  )}
                </div>
                {showInviteForm && (
                  <div className="row-edit-bg" style={{ marginBottom: "16px" }}>
                    <p style={{ margin: "0 0 12px 0", fontWeight: "600", fontSize: "13px", color: "#111" }}>Invite Admin</p>
                    <div className="form-grid-2" style={{ marginBottom: "12px" }}>
                      <div><label className="field-label">Email *</label><input className="adm-input" type="email" value={inviteForm.email} onChange={(e) => { setInviteForm((p) => ({ ...p, email: e.target.value })); setInviteError(""); }} placeholder="email@example.com" /></div>
                      <div><label className="field-label">Full Name</label><input className="adm-input" value={inviteForm.full_name} onChange={(e) => setInviteForm((p) => ({ ...p, full_name: e.target.value }))} placeholder="Optional" /></div>
                    </div>
                    <div style={{ marginTop: "16px", marginBottom: "10px" }}>
                      <p style={{ margin: "0 0 3px 0", fontSize: "11px", fontWeight: "700", color: "#aaa", textTransform: "uppercase", letterSpacing: "0.4px" }}>Permissions</p>
                      <p style={{ margin: 0, fontSize: "12px", color: "#888" }}>You can change these at any time after inviting.</p>
                    </div>
                    <div className="invite-perms-grid" style={{ marginBottom: "14px" }}>
                      {[["can_view_call_sheet","Call Sheet"],["can_edit_prices","Edit Prices"],["can_approve_vets","Approve Vets"],["can_manage_users","Manage Users"],["can_manage_team","Manage Team"]].map(([field, label]) => (
                        <label key={field} className="invite-perm-label"><input type="checkbox" checked={!!inviteForm[field]} onChange={(e) => setInviteForm((p) => ({ ...p, [field]: e.target.checked }))} />{label}</label>
                      ))}
                    </div>
                    {inviteError && <p style={{ margin: "0 0 10px 0", fontSize: "13px", color: "#c62828", fontWeight: "600" }}>⚠️ {inviteError}</p>}
                    <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginTop: "16px" }}>
                      <button className="adm-btn adm-btn-gray" onClick={() => { setShowInviteForm(false); setInviteError(""); }}>Cancel</button>
                      <button className="adm-btn adm-btn-green" onClick={inviteAdminUser} disabled={inviteSaving}>{inviteSaving ? "Saving..." : "Add to Team"}</button>
                    </div>
                  </div>
                )}
                {adminUsersLoading && <p style={{ color: "#888", fontSize: "14px" }}>Loading team...</p>}
                <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: "8px", overflow: "hidden" }}>
                  {adminUsers.length === 0 && !adminUsersLoading && <p style={{ color: "#bbb", fontSize: "14px", padding: "20px", textAlign: "center", fontStyle: "italic" }}>No team members yet.</p>}
                  {adminUsers.map((u, idx) => {
                    const isMe = u.email === currentUserEmail;
                    const isDeactivating = teamDeactivatingId === u.id;
                    return (
                      <div key={u.id} style={{ borderBottom: idx < adminUsers.length - 1 ? "1px solid #f0f0f0" : "none" }}>
                        <div className="team-member-row">
                          <div className="team-member-row-top">
                            <div className="team-member-info">
                              {teamEditingId === u.id ? (
                                <div className="team-edit-name-row" style={{ marginBottom: "6px" }}>
                                  <input className="adm-input team-edit-name-input" value={teamEditName} onChange={(e) => setTeamEditName(e.target.value)} placeholder="Full name..." />
                                  <div className="team-edit-name-btns">
                                    <button className="adm-btn adm-btn-gray team-edit-btn" onClick={() => { setTeamEditingId(null); setTeamEditName(""); }}>Cancel</button>
                                    <button className="adm-btn adm-btn-green team-edit-btn" onClick={() => updateAdminName(u.id, teamEditName)}>Save</button>
                                  </div>
                                </div>
                              ) : (
                                <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", marginBottom: "2px" }}>
                                  <span style={{ fontWeight: "600", fontSize: "14px", color: "#111" }}>{u.full_name || <span style={{ color: "#bbb", fontStyle: "italic" }}>No name</span>}</span>
                                  {isMe && <span style={{ fontSize: "10px", background: "#e3f2fd", color: "#1565c0", padding: "1px 7px", borderRadius: "20px", fontWeight: "700", letterSpacing: "0.3px" }}>YOU</span>}
                                  <span style={{ fontSize: "10px", background: u.status === "active" ? "#e8f5e9" : "#f5f5f5", color: u.status === "active" ? "#2d6a4f" : "#888", padding: "1px 7px", borderRadius: "20px", fontWeight: "600" }}>{u.status === "active" ? "Active" : "Inactive"}</span>
                                  <button className="adm-btn adm-btn-outline" style={{ fontSize: "11px", padding: "2px 8px" }} onClick={() => { setTeamEditingId(u.id); setTeamEditName(u.full_name || ""); setShowInviteForm(false); setInviteError(""); }}>Edit Name</button>
                                </div>
                              )}
                              <p style={{ margin: "0 0 4px 0", fontSize: "12px", color: "#888" }}>{u.email}</p>
                              {u.invited_by && u.invited_by !== "system" && <p style={{ margin: "0 0 8px 0", fontSize: "11px", color: "#aaa" }}>Invited by {u.invited_by}</p>}
                              <p style={{ margin: "14px 0 6px 0", fontSize: "11px", fontWeight: "700", color: "#aaa", textTransform: "uppercase", letterSpacing: "0.4px" }}>Permissions</p>
                              <div className="team-perms-grid">
                                {[["can_view_call_sheet","Call Sheet"],["can_edit_prices","Edit Prices"],["can_approve_vets","Approve Vets"],["can_manage_users","Manage Users"],["can_manage_team","Manage Team"]].map(([field, label]) => {
                                  const locked = isMe && field === "can_manage_team";
                                  const canEdit = adminUsers.find((a) => a.email === currentUserEmail)?.can_manage_team;
                                  return (
                                    <label key={field} style={{ display: "flex", alignItems: "center", gap: "5px", fontSize: "12px", cursor: locked || !canEdit ? "not-allowed" : "pointer", opacity: locked ? 0.4 : 1 }}>
                                      <input type="checkbox" checked={!!u[field]} disabled={locked || !canEdit} onChange={(e) => updateAdminPermission(u.id, field, e.target.checked)} />{label}
                                    </label>
                                  );
                                })}
                              </div>
                          ...
                          {!isMe && adminUsers.find((a) => a.email === currentUserEmail)?.can_manage_team && (
                            <div style={{ marginTop: "20px" }}>
                              {isDeactivating ? (
                                <div style={{ background: "#fff0f0", border: "1px solid #ffcdd2", borderRadius: "6px", padding: "10px 12px" }}>
                                  <p style={{ margin: "0 0 8px 0", fontSize: "13px", color: "#c62828", fontWeight: "600" }}>{u.status === "active" ? "Deactivate" : "Reactivate"} {u.full_name || u.email}?</p>
                                  <div style={{ display: "flex", gap: "6px" }}>
                                    <button className="adm-btn adm-btn-gray" onClick={() => setTeamDeactivatingId(null)}>Cancel</button>
                                    <button className={`adm-btn ${u.status === "active" ? "adm-btn-red" : "adm-btn-green"}`} onClick={() => toggleAdminStatus(u)}>{u.status === "active" ? "Deactivate" : "Reactivate"}</button>
                                  </div>
                                </div>
                              ) : (
                                <button className={`adm-btn ${u.status === "active" ? "adm-btn-gray" : "adm-btn-green"}`} style={{ fontSize: "12px", padding: "5px 10px" }} onClick={() => setTeamDeactivatingId(u.id)}>{u.status === "active" ? "Deactivate" : "Reactivate"}</button>
                              )}
                            </div>
                          )}
                        </div>
                  ...
                  {Object.entries(TRIAGE_CONFIG).map(([key, cfg]) => {
                    ...
                    return (
                      <button key={key} onClick={() => setSymptomFilter(active ? "all" : key)}
                        ...
                        <p className="symptom-stat-label" style={{ margin: 0, fontWeight: "600", color: active ? "#fff" : cfg.color }}>{cfg.label}</p>
                      </button>
                    );
                  })}
                  {symptomFilter !== "all" && <button onClick={() => setSymptomFilter("all")} style={{ background: "#f0f0f0", border: "1px solid #ddd", borderRadius: "10px", padding: "10px 16px", cursor: "pointer", fontSize: "12px", color: "#666", alignSelf: "center" }}>Clear ✕</button>}
                </div>
                {symptomLoading && <p style={{ color: "#888", fontSize: "14px" }}>Loading...</p>}
                <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: "8px", overflow: "hidden" }}>
                  <div className="log-table-header"><span>Pet</span><span>Species</span><span>Result</span><span>Date</span><span>Time</span></div>
                  ...
                  {filteredLogs.map((s) => {
                    ...
                    return (
                      <div key={s.id} className="log-row">
                        <span className="log-col" style={{ fontSize: "14px", fontWeight: "600", color: "#111" }}>{s.pets?.name || <span style={{ color: "#bbb", fontStyle: "italic" }}>Guest</span>}</span>
                        <span className="log-col" style={{ fontSize: "14px", color: "#666", textTransform: "capitalize" }}>{s.pets?.species || "—"}</span>
                        <span className="log-col"><span style={{ fontSize: "12px", fontWeight: "600", color: cfg.color, background: cfg.bg, padding: "3px 10px", borderRadius: "20px", whiteSpace: "nowrap", display: "inline-block" }}>{cfg.label || s.triage_result}</span></span>
                        <span className="log-col" style={{ fontSize: "13px", color: "#555" }}>{formatLogDate(s.created_at)}</span>
                        <span className="log-col" style={{ fontSize: "13px", color: "#888" }}>{formatLogTime(s.created_at)}</span>
                        <div className="log-mobile">
                          {[["Pet:", s.pets?.name || "Guest", "600", "#111"], ["Species:", s.pets?.species || "—", "400", "#666"], ["Date:", formatLogDate(s.created_at), "400", "#555"], ["Time:", formatLogTime(s.created_at), "400", "#888"]].map(([lbl, val, fw, clr]) => (
                            <div key={lbl} style={{ display: "flex", alignItems: "baseline", gap: "6px" }}>
                              <span style={{ fontSize: "10px", color: "#aaa", fontWeight: "700", textTransform: "uppercase", letterSpacing: "0.4px", flexShrink: 0 }}>{lbl}</span>
                              <span style={{ fontSize: "13px", fontWeight: fw, color: clr, textTransform: lbl === "Species:" ? "capitalize" : "none" }}>{val}</span>
                            </div>
                          ))}
                          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                            <span style={{ fontSize: "10px", color: "#aaa", fontWeight: "700", textTransform: "uppercase", letterSpacing: "0.4px", flexShrink: 0 }}>Result:</span>
                            <span style={{ fontSize: "11px", fontWeight: "600", color: cfg.color, background: cfg.bg, padding: "2px 10px", borderRadius: "20px", whiteSpace: "nowrap" }}>{cfg.label || s.triage_result}</span>
                          </div>
                        </div>
                      </div>
                    );
      ...
      {unsavedModal && (
        <>
          <div onClick={() => setUnsavedModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 600 }} />
          <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 700, background: "#fff", borderRadius: "12px", padding: "24px", width: "min(380px, 90vw)", boxShadow: "0 8px 32px rgba(0,0,0,0.18)", fontFamily: "system-ui, sans-serif" }}>
            <p style={{ margin: "0 0 6px 0", fontSize: "16px", fontWeight: "700", color: "#111" }}>Unsaved Prices</p>
            <p style={{ margin: "0 0 20px 0", fontSize: "14px", color: "#555" }}>{unsavedModal.message}</p>
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px" }}>
              <button className="adm-btn adm-btn-gray" onClick={() => setUnsavedModal(null)}>Cancel — Keep Editing</button>
              <button className="adm-btn adm-btn-red" onClick={() => { unsavedModal.action(); setUnsavedModal(null); }}>Discard & Continue</button>
            </div>
          </div>
        </>
      )}
      ...
      {/* ── Global Notes Panel ── */}
      <div className={`notes-panel-desktop${showAllNotes ? " notes-panel-open" : ""}`}
        style={{ position: "fixed", zIndex: 500, background: "#fff", boxShadow: "0 0 32px rgba(0,0,0,0.18)", transition: "transform 0.3s ease", overflowY: "auto", fontFamily: "system-ui, sans-serif" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px", borderBottom: "1px solid #ffe0b2", background: "#fff8e1", position: "sticky", top: 0 }}>
          <span style={{ fontSize: "14px", fontWeight: "700", color: "#e65100" }}>📋 All Call Notes {allCallNotes.length > 0 ? `(${allCallNotes.length})` : ""}</span>
          <button onClick={() => setShowAllNotes(false)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "20px", color: "#aaa", lineHeight: 1 }}>✕</button>
        </div>
        <div style={{ padding: "0" }}>
          {allNotesLoading && <p style={{ padding: "16px", color: "#888", fontSize: "13px", margin: 0 }}>Loading notes...</p>}
          {!allNotesLoading && allCallNotes.length === 0 && <p style={{ padding: "16px", color: "#aaa", fontSize: "13px", fontStyle: "italic", margin: 0 }}>No notes saved yet.</p>}
          {!allNotesLoading && allCallNotes.map((n) => (
            <div key={n.id} style={{ padding: "14px 20px", borderBottom: "1px solid #f0f0f0" }}>
              {allNotesEditingId === n.id ? (
                <div>
                  <p style={{ margin: "0 0 6px 0", fontSize: "12px", fontWeight: "600", color: "#2d6a4f" }}>{n.vet_name}</p>
                  <textarea className="adm-input" rows={3} style={{ width: "100%", resize: "vertical", height: "auto", marginBottom: "8px" }} value={allNotesEditingText} onChange={(e) => setAllNotesEditingText(e.target.value)} />
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px" }}>
                    <button className="adm-btn adm-btn-gray" onClick={() => { setAllNotesEditingId(null); setAllNotesEditingText(""); }}>Cancel</button>
                    <button className="adm-btn adm-btn-green" onClick={() => updateCallNote(n.id, allNotesEditingText)}>Save Changes</button>
                  </div>
                </div>
              ) : allNotesDeletingId === n.id ? (
                <div style={{ background: "#fff0f0", border: "1px solid #ffcdd2", borderRadius: "6px", padding: "12px" }}>
                  <p style={{ margin: "0 0 4px 0", fontSize: "12px", fontWeight: "600", color: "#111" }}>{n.vet_name}</p>
                  <p style={{ margin: "0 0 10px 0", fontSize: "13px", color: "#c62828", fontWeight: "600" }}>Delete this note?</p>
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px" }}>
                    <button className="adm-btn adm-btn-gray" onClick={() => setAllNotesDeletingId(null)}>Cancel</button>
                    <button className="adm-btn adm-btn-red" onClick={() => deleteCallNote(n.id)}>Yes, Delete</button>
                  </div>
                </div>
              ) : (
                <div>
                  <p style={{ margin: "0 0 2px 0", fontSize: "13px", fontWeight: "600", color: "#111" }}>{n.vet_name}</p>
                  <p style={{ margin: "0 0 6px 0", fontSize: "11px", color: "#aaa" }}>{formatLogDate(n.created_at)}{n.updated_at !== n.created_at ? " · edited" : ""}</p>
                  <p style={{ margin: "0 0 10px 0", fontSize: "13px", color: "#555", lineHeight: "1.5" }}>{n.note}</p>
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px" }}>
                    <button className="adm-btn adm-btn-outline" onClick={() => { setAllNotesEditingId(n.id); setAllNotesEditingText(n.note); setAllNotesDeletingId(null); }}>Edit</button>
                    <button className="adm-btn adm-btn-red" onClick={() => { setAllNotesDeletingId(n.id); setAllNotesEditingId(null); }}>Delete</button>
                  </div>
                </div>
              )}
            </div>
      ...
      <div style={{ position: "fixed", bottom: "24px", right: "19px", zIndex: 300, display: "flex", flexDirection: "column", gap: "8px", alignItems: "center" }}>
        <button onClick={() => { setShowAllNotes((v) => !v); if (!showAllNotes) fetchAllCallNotes(); }}
          style={{ width: "40px", height: "40px", borderRadius: "50%", background: "#e65100", color: "#fff", border: "none", cursor: "pointer", fontSize: "18px", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 8px rgba(230,81,0,0.35)", fontFamily: "system-ui, sans-serif" }}
          title="Call Notes">📋</button>
        <button className="scroll-arrow-btn" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} title="Scroll to top">↑</button>
        <button className="scroll-arrow-btn" onClick={() => window.scrollTo({ top: document.body.scrollHeight, behavior: "smooth" })} title="Scroll to bottom">↓</button>
      </div>
      {showAllNotes && <div onClick={() => setShowAllNotes(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.3)", zIndex: 400 }} />}
    </>
  );
}
