...
export default function AdminPage() {
  ...
  async function saveCallPrices(vet) {
    ...
    if (callPrices.length > 0 && validPrices.length < callPrices.length) { alert('Please select a species for every price row.'); setCallSaving(false); return; }
    ...
    for (const p of validPrices) {
      const { data, error } = await supabase.from("vet_prices").insert(cleanPrice(p, savedVetId)).select().single();
  ...
  async function fetchPricesForVet(vetId) {
    ...
    const { data } = await supabase.from("vet_prices")
      ...
      <div style={{ maxWidth: "900px", margin: "0 auto", padding: "20px", fontFamily: "system-ui, sans-serif", minHeight: "100vh", background: "#f7f7f5" }}>
        ...
        <div style={{ background: "#fff", borderRadius: "12px", border: "1px solid #e8e8e8" }}>
          <div className="tabs-scroll" style={{ position: "relative" }}>
            {TABS.map(t => (
              <button key={t} className={`tab-btn${tab === t ? " active" : ""}`} onClick={() => { setTab(t); if (t === "Prices" && selectedVetId) fetchPricesForVet(selectedVetId); }}>
                {t}
                          ...
                          <div>
                            ...
                            <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "8px" }}>
                              {[["includes_bloodwork","Bloodwork"],["includes_xrays","X-rays"],["includes_anesthesia","Anesthesia"]].map(([f, l]) => (
                                <button key={f} type="button" onClick={() => setAddPriceForm(prev => ({ ...prev, [f]: !prev[f] }))} style={{ padding: "3px 8px", borderRadius: "20px", fontSize: "11px", fontWeight: "600", cursor: "pointer", border: addPriceForm[f] ? "none" : "1px solid #ddd", background: addPriceForm[f] ? "#2d6a4f" : "#f5f5f5", color: addPriceForm[f] ? "#fff" : "#555", transition: "all 0.15s" }}>{l}</button>
                              ))}
                            </div>
                              ...
                              <div className="row-edit-bg">
                                ...
                                <div className="form-grid-3" style={{ marginBottom: "10px" }}>
                                  ...
                                  <div>
                                    ...
                                    <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "8px" }}>
                                      {[["includes_bloodwork","Bloodwork"],["includes_xrays","X-rays"],["includes_anesthesia","Anesthesia"]].map(([f, l]) => (
                                        <button key={f} type="button" onClick={() => setPriceForm(prev => ({ ...prev, [f]: !prev[f] }))} style={{ padding: "3px 8px", borderRadius: "20px", fontSize: "11px", fontWeight: "600", cursor: "pointer", border: priceForm[f] ? "none" : "1px solid #ddd", background: priceForm[f] ? "#2d6a4f" : "#f5f5f5", color: priceForm[f] ? "#fff" : "#555", transition: "all 0.15s" }}>{l}</button>
                                      ))}
                                    </div>
                                    <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", cursor: "pointer" }}>
                  ...
                  return (
                    <div>
                      ...
                      <div className="call-sheet-header" style={{ marginBottom: "16px" }}>
                        ...
                        <div
                          onClick={() => { setShowAllVets(v => !v); setCallIndex(0); setCallPrices([]); setCallReviewPrices([]); setCallReviewVetId(null); setCallSaved(false); }}
                          style={{ position: "relative", display: "flex", borderRadius: "20px", border: "1px solid #2d6a4f", overflow: "hidden", flexShrink: 0, cursor: "pointer", background: "#fff", userSelect: "none", width: "160px" }}
                        >
                          <div style={{ position: "absolute", top: 0, bottom: 0, width: "50%", background: "#2d6a4f", borderRadius: "18px", transition: "transform 0.25s ease", transform: showAllVets ? "translateX(100%)" : "translateX(0%)" }} />
                          <span style={{ position: "relative", flex: 1, textAlign: "center", padding: "6px 0", fontSize: "12px", fontWeight: "600", color: !showAllVets ? "#fff" : "#2d6a4f", transition: "color 0.25s ease", zIndex: 1 }}>Unpriced</span>
                          <span style={{ position: "relative", flex: 1, textAlign: "center", padding: "6px 0", fontSize: "12px", fontWeight: "600", color: showAllVets ? "#fff" : "#2d6a4f", transition: "color 0.25s ease", zIndex: 1 }}>All Vets</span>
                        </div>
                        <div style={{ display: "flex", gap: "8px" }}>
                      ...
                      <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: "12px", padding: "20px", marginBottom: "16px" }}>
                        ...
                        <div>
                          ...
                          {callPrices.map((p, i) => (
                            <div key={i} className="row-edit-bg" style={{ marginBottom: "12px" }}>
                              {/* Row 1: Service + Price Type + Low + High */}
                              <div className="form-grid-4" style={{ marginBottom: "14px" }}>
                                <div><label className="field-label">Service</label><select className="adm-input" value={p.service_id} onChange={e => updateCallPrice(i, "service_id", e.target.value)}><option value="">— Select —</option>{services.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
                                ...
                                <div><label className="field-label">Price Low</label><input className="adm-input" type="number" value={p.price_low} onChange={e => updateCallPrice(i, "price_low", e.target.value)} placeholder="e.g. 65" /></div>
                              ...
                              {/* Row 1b: Species — half width, same as Service */}
                              <div className="form-grid-2" style={{ marginBottom: "14px" }}>
                                <div>
                                  ...
                                  <select className="adm-input" value={p.species || ""} onChange={e => setCallPrices(prev => prev.map((row, idx) => idx === i ? { ...row, species: e.target.value } : row))}>
                              ...
                              <div style={{ marginBottom: "14px" }}>
                                ...
                                <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                                  {[["includes_bloodwork","Bloodwork"],["includes_xrays","X-rays"],["includes_anesthesia","Anesthesia"]].map(([field, label]) => (
                                    <button key={field} type="button" onClick={() => setCallPrices(prev => prev.map((row, idx) => idx === i ? { ...row, [field]: !row[field] } : row))} style={{ padding: "3px 8px", borderRadius: "20px", fontSize: "11px", fontWeight: "600", cursor: "pointer", border: p[field] ? "none" : "1px solid #ddd", background: p[field] ? "#2d6a4f" : "#f5f5f5", color: p[field] ? "#fff" : "#555", transition: "all 0.15s" }}>{label}</button>
                                  ))}
                                </div>
                              </div>
                              {/* Row 3: Notes full width */}
                              <div style={{ marginBottom: "14px" }}>
                                <input className="adm-input" style={{ width: "100%" }} value={p.notes} onChange={e => updateCallPrice(i, "notes", e.target.value)} placeholder="Notes..." />
                              </div>
                              {/* Row 4: Clear + Remove buttons — left aligned */}
                              <div style={{ display: "flex", justifyContent: "flex-start", gap: "8px", alignItems: "center" }}>
                                <button className="adm-btn adm-btn-gray" onClick={() => { const u=[...callPrices]; u[i]={...u[i], service_id:"", price_low:"", price_high:"", price_type:"exact", includes_bloodwork:false, includes_xrays:false, includes_anesthesia:false, notes:""}; setCallPrices(u); }}>Clear</button>
                          ...
                          {callPrices.length > 0 && (
                            <div style={{ marginTop: "8px" }}>
                              <button className="adm-btn adm-btn-green" style={{ padding: "10px 24px", fontSize: "14px", width: "100%" }} onClick={() => saveCallPrices(vet)} disabled={callSaving}>
                                ...
                                {!isEditing ? (
                                  ...
                                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "12px" }}>
                                    <div style={{ flex: 1 }}>
                                      ...
                                      {row.notes && <p style={{ margin: "4px 0 0 0", fontSize: "12px", color: "#888", fontStyle: "italic" }}>{row.notes}</p>}
                                    </div>
                                    <div style={{ display: "flex", gap: "6px", flexShrink: 0 }}>
                                  ...
                                  <div style={{ background: "#f9f9f9", borderRadius: "8px", padding: "12px" }}>
                                    <div className="form-grid-4" style={{ marginBottom: "14px" }}>
                                      ...
                                      <div><label className="field-label">Price Type</label><select className="adm-input" value={row.price_type} onChange={e => { const u=[...callReviewPrices]; u[i]={...u[i],price_type:e.target.value}; setCallReviewPrices(u); }}>{PRICE_TYPES.map(t=><option key={t}>{t}</option>)}</select></div>
                                      <div><label className="field-label">Price Low</label><input className="adm-input" type="number" value={row.price_low||""} onChange={e => { const u=[...callReviewPrices]; u[i]={...u[i],price_low:e.target.value}; setCallReviewPrices(u); }} /></div>
                                      <div><label className="field-label">Price High</label><input className="adm-input" type="number" value={row.price_high||""} onChange={e => { const u=[...callReviewPrices]; u[i]={...u[i],price_high:e.target.value}; setCallReviewPrices(u); }} /></div>
                                    </div>
                                    <div style={{ marginBottom: "8px" }}>
                                      <label className="field-label" style={{ display: "block", marginBottom: "6px" }}>Includes</label>
                                      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                                        {[["includes_bloodwork","Bloodwork"],["includes_xrays","X-rays"],["includes_anesthesia","Anesthesia"]].map(([field, label]) => (
                                          <button key={field} type="button" onClick={() => { const u=[...callReviewPrices]; u[i]={...u[i],[field]:!u[i][field]}; setCallReviewPrices(u); }} style={{ padding: "3px 8px", borderRadius: "20px", fontSize: "11px", fontWeight: "600", cursor: "pointer", border: row[field] ? "none" : "1px solid #ddd", background: row[field] ? "#2d6a4f" : "#f5f5f5", color: row[field] ? "#fff" : "#555", transition: "all 0.15s" }}>{label}</button>
                                        ))}
                                      </div>
                                    </div>
                                    <div style={{ marginBottom: "8px" }}><input className="adm-input" style={{ width: "100%" }} value={row.notes||""} onChange={e => { const u=[...callReviewPrices]; u[i]={...u[i],notes:e.target.value}; setCallReviewPrices(u); }} placeholder="Notes..." /></div>
            ...
            {tab === "Users" && (
              <div>
                ...
                <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: "8px", overflow: "hidden" }}>
                  ...
                  {filteredUsers.map(u => (
                    <div key={u.id} className="users-row" style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", padding: "14px 16px", borderBottom: "1px solid #f5f5f3", alignItems: "start", gap: "8px" }}>
                      <div>
                        <span style={{ fontSize: "14px", fontWeight: "600", color: "#111" }}>{u.full_name || <span style={{ color: "#bbb", fontStyle: "italic" }}>No name</span>}</span>
                        {u.bio && <p style={{ margin: "3px 0 0 0", fontSize: "13px", color: "#888", overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{u.bio}</p>}
                      </div>
                      <span style={{ fontSize: "14px", color: "#666" }}>{u.zip_code || "—"}</span>
                      <span style={{ fontSize: "13px" }}>{u.is_public ? <span style={{ color: "#2d6a4f" }}>✅ Public</span> : <span style={{ color: "#bbb" }}>Private</span>}</span>
                      <span style={{ fontSize: "13px", color: "#888" }}>{formatDate(u.created_at)}</span>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  {Object.entries(TRIAGE_CONFIG).map(([key, cfg]) => {
                    ...
                    return (
                      <button key={key} onClick={() => setSymptomFilter(active ? "all" : key)}
                        style={{ background: active ? cfg.color : cfg.bg, border: `1px solid ${cfg.color}44`, borderRadius: "10px", padding: "10px 14px", cursor: "pointer", textAlign: "left", transition: "all 0.15s" }}>
                        ...
                        <p style={{ margin: 0, fontSize: "11px", fontWeight: "600", color: active ? "#fff" : cfg.color, whiteSpace: "nowrap" }}>{cfg.label}</p>
                      </button>
                    );
                  })}
                  {symptomFilter !== "all" && (
                  ...
                  {filteredLogs.map(s => {
                    ...
                    return (
                      <div key={s.id} className="log-row">
                        ...
                        <span style={{ fontSize: "13px", color: "#aaa" }}>{formatDateTime(s.created_at)}</span>
                      </div>
                    );
                  })}
                </div>
