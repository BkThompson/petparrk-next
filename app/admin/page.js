...
export default function AdminPage() {
  ...
  async function updateSubmissionStatus(id, status) {
    ...
    // When approving: insert into vet_prices so it actually shows on the vet profile
    if (status === "approved") {
      ...
      // Resolve vet_id — prefer stored vet_id, fall back to name lookup
      ...
      if (vetId && serviceId) {
        const { error } = await supabase.from("vet_prices").insert({
          ...
          price_low: sub.price_paid ? parseFloat(sub.price_paid) : null,
          price_type: "exact",
          is_verified: true,
          source: "community_submission",
          notes: sub.submitter_note || null,
        });
        if (error) {
          console.error(
            "Failed to insert vet_price from submission:",
            error.message,
          );
          alert(`Submission approved but price entry failed: ${error.message}`);
        }
      } else {
        // Could not match vet or service — alert admin to add manually
        const missing = [];
        if (!vetId) missing.push("vet not found in database");
        if (!serviceId)
          missing.push(
            `service "${sub.service_name}" not found in services table`,
          );
        alert(
          `Submission marked approved but price NOT added automatically:\n${missing.join("\n")}\n\nPlease add it manually via the Prices tab.`,
        );
      }
    }
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              fontWeight: "600",
              cursor: "pointer",
              border: form[f] ? "none" : "1px solid #ddd",
              background: form[f] ? "#2d6a4f" : "#f5f5f5",
              color: form[f] ? "#fff" : "#555",
              whiteSpace: "nowrap",
            }}
          >
            {l}
                    ...
                    >
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder="e.g. clinicname.com"
                        />
                      </div>
                    </div>
                    <div
                    ...
                    return (
                      <div key={s.id} className="log-row">
                        ...
                        <div className="log-mobile">
                          {[
                            ["Pet:", s.pets?.name || "Guest", "600", "#111"],
                            ["Species:", s.pets?.species || "—", "400", "#666"],
                            [
                              "Date:",
                              formatLogDate(s.created_at),
                              "400",
