...
export function profileCss({ C, bannerKey, bannerPalette }) {
  ...
  .field-hint { margin: 4px 0 0; font-size: 14px; font-weight: 500; color: ${C.muted}; }
  .btn-primary {
  ...
  .pp-banner-swatch-color svg {
    ...
    /* Sized here rather than via each icon's size prop: the swatches are
       rendered from more than one component, and CSS overrides the width and
       height Lucide writes onto the element, so this reaches all of them. */
    width: 35px;
    height: 35px;
  }
